...
`xAxis`: saluran sumbu X, mendukung `satu dimensi`, dimasukkan dan dihitung berdasarkan nilai dimensi dan ditampilkan pada sumbu X

:::

:::note{title="Deskripsi"}
Histogram cocok untuk skenario yang menunjukkan distribusi data. Sumbu X adalah sumbu numerik (data kontinu), sumbu Y adalah sumbu numerik (data kontinu), dan kolom-kolom disusun secara vertikal.

Skenario penggunaan:

\- Menampilkan distribusi data, seperti distribusi frekuensi, distribusi probabilitas, dll.

\-Analisis tendensi sentral dan sebaran data
...
:::note{title="Deskripsi"}
Histogram, cocok untuk menampilkan sebaran data
...
**Tipe:** `HistogramDimension[] | undefined`
...
**Tipe:** `"tooltip" | "label" | "row" | "column" | undefined`
...
**Tipe:** `HistogramMeasure[] | undefined`

:::note{title="Deskripsi"}
Histogram hanya mendukung satu dimensi, dan datanya merupakan data diskrit
...
**Tipe:** `"value" | "color" | "tooltip" | "label" | "x0" | "x1" | undefined`
...
\-x0: x0 saluran histogram

\-x1: x1 saluran histogram
...
:::note{title="Deskripsi"}
Nomor binning histogram, digunakan untuk menentukan jumlah binning persegi panjang (kolom) histogram.
...
:::note{title="Deskripsi"}
Ukuran langkah binning, yang digunakan untuk menghitung lebar bin, juga mempengaruhi lebar persegi panjang (kolom) pada histogram akhir. Jika binCount dan binStep disetel, binStep akan diutamakan.
...
**Tipe:** `"count" | "percentage" | undefined`

:::note{title="Deskripsi"}
Tipe nilai binning histogram, digunakan untuk menentukan tipe nilai binning persegi panjang (kolom) dari histogram, defaultnya adalah 'count'
...
:::note{title="Deskripsi"}
Garis label numerik (nilai binning) ditampilkan dalam arah vertikal. Posisi dan gaya garis label dapat diatur. Jika Anda memerlukan baris label yang sesuai dengan nilai binning, Anda dapat menggunakan konfigurasi ini.
...
:::note{title="Deskripsi"}
Garis label numerik (termasuk garis rata-rata, garis nilai maksimum, garis nilai minimum, dll.) ditampilkan dalam arah horizontal. Posisi dan gaya garis label dapat diatur. Jika Anda perlu menggambar garis label yang sesuai dengan nilai binning, silakan gunakan konfigurasi ini; perhatikan bahwa nilai binning dipengaruhi oleh `binValueType`
...
**Tipe:** `KdeRegressionLine | KdeRegressionLine[] | undefined`
...
:::note{title="Deskripsi"}
Apakah akan mengaktifkan fungsi garis regresi
...
**Tipe:** `EcdfRegressionLine | EcdfRegressionLine[] | undefined`

:::note{title="Deskripsi"}
Konfigurasi garis regresi fungsi distribusi kumulatif empiris, digunakan untuk menampilkan sebaran data kumulatif
