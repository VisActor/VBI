...
Diagram batang berdampingan mendukung saluran visual berikut:

`yAxis`: saluran sumbu Y, mendukung `beberapa dimensi`, dipetakan ke sumbu Y sesuai dengan nilai dimensi

`xAxis`: saluran sumbu X, mendukung `beberapa metrik`, dipetakan ke sumbu X sesuai dengan nilai metrik
...
Grafik batang berdampingan cocok untuk skenario perbandingan paralel horizontal beberapa metrik. Beberapa batang disusun secara paralel untuk menampilkan nilai metrik yang berbeda.

Skenario penggunaan:

\-Perbandingan beberapa metrik ketika nama kategori panjang

\-Perbandingan horizontal di mana peringkat dan nilai ditampilkan secara bersamaan

\- Analisis data multidimensi secara berdampingan
...
\- setidaknya 1 field metrik (metrik)

\- Dimensi pertama akan ditempatkan pada sumbu Y, dan dimensi lainnya akan digabungkan dengan nama metrik (bila terdapat beberapa metrik) dan ditampilkan sebagai item legenda.
...
:::note{title="Deskripsi"}
Bagan batang berdampingan, cocok untuk skenario perbandingan paralel horizontal multi-metrik
...
:::note{title="Deskripsi"}
Sumber data, kumpulan data gabungan yang sesuai dengan spesifikasi TidyData, digunakan untuk menentukan sumber data dan struktur bagan. Kumpulan data yang dimasukkan oleh pengguna tidak memerlukan pemrosesan apa pun. VSeed memiliki fungsi pembentukan ulang data yang kuat dan akan membentuk ulang data dengan sendirinya. Data diagram batang berdampingan pada akhirnya akan diubah menjadi 2 dimensi dan 1 metrik.
...
**Tipe:** `BarDimension[] | undefined`

:::note{title="Deskripsi"}
Dimensi, dimensi pertama dipetakan ke sumbu Y, dan dimensi lainnya akan digabungkan dengan nama metrik (bila terdapat beberapa metrik) dan ditampilkan sebagai item legenda.
...
**Tipe:** `"color" | "detail" | "tooltip" | "label" | "row" | "column" | "yAxis" | undefined`
...
**Tipe:** `BarMeasure[] | undefined`

:::note{title="Deskripsi"}
Metrik, metrik diagram batang yang berdampingan akan secara otomatis digabungkan menjadi satu metrik dan dipetakan ke sumbu X. Jika terdapat beberapa metrik, nama metrik akan digabungkan dengan dimensi lain dan ditampilkan sebagai item legenda.

:::

**Contoh**
```ts
[{id: 'nilai1', alias: 'Metrik 1'}, {id: 'nilai2', alias: 'Metrik 2'}]
...
**Tipe:** `"xAxis" | "color" | "detail" | "tooltip" | "label" | undefined`

:::note{title="Deskripsi"}
Saluran pemetaan metrik

\- xAxis: pemetaan metrik sumbu X
...
Konfigurasi paging, digunakan untuk mengkonfigurasi fungsi paging grafik
...
:::note{title="Deskripsi"}
Warna latar belakang bagan, yang defaultnya adalah latar belakang transparan. Warna latar belakang dapat berupa string warna, seperti 'merah', 'biru', atau hex, rgb atau rgba'#ff0000', 'rgba(255,0,0,0.5)'
...
**Tipe:** `RegionPadding | undefined`

:::note{title="Deskripsi"}
Bantalan area gambar



Region[0].padding yang dipetakan ke VChart digunakan untuk menyediakan ruang bagi elemen perluasan eksternal pada area gambar seperti anotasi dan label.
...
**Tipe:** `BarLikeAnimation | undefined`
...
Konfigurasi animasi bagan, efek opsional dibatasi oleh jenis bagan
...
:::note{title="Deskripsi"}
Apakah akan mengaktifkan animasi bar/kolom

:::

### params

**Tipe:** `BarLikeAnimationParams | undefined`

:::note{title="Deskripsi"}
Parameter animasi bagan batang/kolom
...
**Tipe:** `BarLikeAppearAnimation | undefined`

:::note{title="Deskripsi"}
Konfigurasi animasi entri bar/kolom
...
:::note{title="Deskripsi"}
Efek entri bagan batang/kolom, mendukung animasi pertumbuhan
...
:::note{title="Deskripsi"}
Apakah akan mengaktifkan tahap animasi saat ini
...
:::note{title="Deskripsi"}
Sorotan animasi atau warna suasana hati

:::

#### update

**Tipe:** `BarLikeUpdateAnimation | undefined`

:::note{title="Deskripsi"}
Konfigurasi animasi pembaruan bagan batang/kolom
...
**Tipe:** `("growth" | "moveIn")[] | undefined`

:::note{title="Deskripsi"}
Efek pembaruan bagan batang/kolom, mendukung pertumbuhan dan animasi bergerak
...
**Tipe:** `BarLikeAnimationLoop | undefined`

:::note{title="Deskripsi"}
Konfigurasi animasi siklus batang/kolom
...
:::note{title="Deskripsi"}
Apakah akan mengaktifkan animasi loop
...
**Tipe:** `BarLikeLoopAnimation | undefined`
...
**Tipe:** `BarLikeLoopEffect[] | undefined`

:::note{title="Deskripsi"}
Efek lingkaran bagan batang/kolom
...
:::note{title="Deskripsi"}
Konfigurasi animasi suasana bagan batang/kolom
...
:::note{title="Deskripsi"}
Fungsi pelonggaran animasi suasana
...
:::note{title="Deskripsi"}
Efek animasi atmosfer, mendukung riak, bersembunyi dan bernapas
...
:::note{title="Deskripsi"}
sumbu X, sumbu nilai, konfigurasi sumbu X, digunakan untuk menentukan sumbu X pada bagan, termasuk posisi, format, gaya, dll. dari sumbu X.
...
**Tipe:** `YBandAxis | undefined`

:::note{title="Deskripsi"}
sumbu Y, sumbu kategori, konfigurasi sumbu Y, digunakan untuk menentukan sumbu Y pada bagan, termasuk posisi, format, gaya, dll. dari sumbu Y.
...
:::note{title="Deskripsi"}
Konfigurasi kotak prompt horizontal digunakan untuk menentukan kotak prompt horizontal pada bagan, termasuk warna kotak prompt horizontal, gaya label, dll.
...
:::note{title="Deskripsi"}
Bagan batang bertumpuk dengan sudut membulat
...
:::note{title="Deskripsi"}
Tinggi maksimum persegi panjang, yang dapat berupa nilai piksel atau string persentase
...
:::note{title="Deskripsi"}
Dalam kategori yang sama, jarak antar persegi panjang dapat berupa nilai piksel atau string persentase.
...
:::note{title="Deskripsi"}
Konfigurasi penyortiran sumbu Y, mendukung penyortiran berdasarkan dimensi atau metrik, dan urutan penyortiran khusus
...
:::note{title="Deskripsi"}
Gaya primitif persegi panjang, konfigurasi gaya diagram batang, digunakan untuk menentukan gaya diagram batang pada diagram, termasuk warna diagram batang, batas, sudut membulat, dll.
...
:::note{title="Deskripsi"}
Apakah kolom primitif (primitif persegi panjang) terlihat
...
:::note{title="Deskripsi"}
Warna kolom primitif (primitif persegi panjang).
...
:::note{title="Deskripsi"}
Transparansi warna primitif kolom (primitif persegi panjang).
...
:::note{title="Deskripsi"}
Warna batas kolom primitif (primitif persegi panjang).
...
:::note{title="Deskripsi"}
Lebar batas kolom primitif (primitif persegi panjang).
...
:::note{title="Deskripsi"}
Gaya batas kolom primitif (primitif persegi panjang).
...
:::note{title="Deskripsi"}
Kolom primitif (primitif persegi panjang) sudut membulat



Transparansi guratan kolom primitif (primitif persegi panjang).
...
:::note{title="Deskripsi"}
Garis label numerik (termasuk garis rata-rata, garis nilai maksimum, garis nilai minimum, dll.) ditampilkan dalam arah vertikal. Posisi dan gaya garis label dapat diatur. Jika Anda perlu menggambar garis label yang sesuai dengan nilai seperti garis rata-rata, silakan gunakan konfigurasi ini.
...
:::note{title="Deskripsi"}
Garis label nilai dimensi, ditampilkan secara horizontal, dapat mengatur posisi, gaya, dll. dari garis label
...
**Tipe:** `AnnotationDifferenceLine | AnnotationDifferenceLine[] | undefined`

:::note{title="Deskripsi"}
Konfigurasi garis label perbedaan, digunakan untuk mengikat dua titik jangkar data dan menampilkan perbedaan absolut atau persentase perbedaan.
...
**Tipe:** `DifferenceAnchor`

:::note{title="Deskripsi"}
Titik jangkar awal dari garis dimensi perbedaan.



Konfigurasi titik jangkar anotasi diferensial, digunakan untuk memilih data yang akan diikat oleh titik awal atau titik akhir.
...
**Tipe:** `DifferenceSelector | DifferenceSelector[]`

:::note{title="Deskripsi"}
Pemilih titik jangkar pada akhirnya harus diposisikan pada titik jangkar yang logis.
...
:::note{title="Deskripsi"}
Titik jangkar akhir dari garis dimensi diferensial.
...
**Tipe:** `"percent" | "absolute" | undefined`

:::note{title="Deskripsi"}
Jenis nilai perbedaan.

\- absolut: menampilkan selisih absolut, dihitung sebagai akhir \- awal

\- persen: Menampilkan selisih persentase, dihitung sebagai (akhir \- awal) / awal
