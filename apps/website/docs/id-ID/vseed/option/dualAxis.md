...
\- Konfigurasi field yang direkomendasikan: metrik `2`, dimensi `2`
...
Bagan sumbu ganda mendukung saluran visual berikut:

`xAxis`: saluran sumbu X, mendukung `beberapa dimensi`, dipetakan ke sumbu X sesuai dengan nilai dimensi

`primaryYAxis`: Saluran sumbu Y sumbu utama, mendukung `beberapa metrik`, memetakan metrik ke sumbu utama

`secondaryYAxis`: saluran sumbu Y sumbu sekunder, mendukung `beberapa metrik`, memetakan metrik ke sumbu sekunder
...
Grafik sumbu ganda cocok untuk menunjukkan hubungan komparatif antara dua metrik dengan besaran atau satuan berbeda, termasuk sumbu koordinat primer dan sumbu koordinat sekunder.

Skenario penggunaan:

\- Analisis komparatif metrik dengan besaran berbeda

\- Perbandingan tren metrik korelasi

\- Metrik gabungan seperti nilai numerik dan tingkat pertumbuhan perlu ditampilkan secara bersamaan

\- Mendukung berbagai jenis kombinasi grafik (seperti diagram garis + diagram batang/diagram garis+diagram area/diagram area+diagram batang)
...
\-Kelompok metrik pendukung, kelompok metrik pertama akan ditempatkan pada sumbu kiri (primer), dan kelompok metrik kedua akan ditempatkan pada sumbu kanan (sekunder)
...
\- Dua kumpulan field metrik dapat dipetakan masing-masing ke sumbu Y kiri dan kanan, dan semua metrik dalam grup metrik akan secara otomatis digabungkan menjadi satu metrik.

Fitur yang diaktifkan secara default:

\- Sumbu, legenda, label data, dan informasi cepat diaktifkan secara default.
...
:::note{title="Deskripsi"}
Grafik sumbu ganda, grafik gabungan yang menunjukkan hubungan komparatif antara dua metrik yang besarannya berbeda
...
:::note{title="Deskripsi"}
Kumpulan data, kumpulan data yang sesuai dengan spesifikasi TidyData dan telah dikumpulkan, digunakan untuk menentukan sumber data dan struktur bagan. Kumpulan data yang dimasukkan oleh pengguna tidak memerlukan pemrosesan apa pun. VSeed memiliki fungsi pembentukan ulang data yang kuat dan akan membentuk ulang data dengan sendirinya. Data grafik sumbu ganda pada akhirnya akan diubah menjadi 2 dimensi, 1 atau 2 metrik (tergantung apakah pengguna telah mengonfigurasi grup metrik).

:::

**Contoh**
```ts
[{bulan:'Januari', value:100, pertumbuhan:0,2}, {bulan:'Februari', value:150, pertumbuhan:0,5}]
...
**Tipe:** `ColumnDimension[] | undefined`

:::note{title="Deskripsi"}
Dimensi, dimensi pertama akan ditempatkan pada sumbu X, dan dimensi lainnya akan digabungkan dengan nama metrik (bila terdapat beberapa metrik) dan ditampilkan sebagai item legenda.

:::

**Contoh**
```ts
[{id: 'bulan', alias: 'bulan'}]
...
**Tipe:** `"xAxis" | "color" | "detail" | "tooltip" | "label" | "row" | "column" | undefined`
...
**Tipe:** `DualAxisMeasure[] | undefined`

:::note{title="Deskripsi"}
Metrik Grafik Sumbu Ganda

Untuk metrik yang dipetakan ke sumbu Y primer dan sumbu Y sekunder dalam pengkodean,

Anda dapat mengelompokkan metrik dengan mengatur atribut `parentId`, dan metrik dalam grup berbeda akan ditampilkan dalam subgraf berbeda.

Anda juga dapat mengatur atribut `chartType` untuk menentukan tipe grafik untuk grup metrik yang berbeda.

:::

**Contoh**
```ts
[{ id: 'value', encoding: 'primarySumbu Y' }, { id: 'growth', encoding: 'secondarySumbu Y' }]
...
**Tipe:** `"color" | "tooltip" | "label" | "primarySumbu Y" | "secondarySumbu Y" | undefined`

:::note{title="Deskripsi"}
Saluran pemetaan metrik

\- primarySumbu Y: sumbu Y utama dari pemetaan metrik

\- secondarySumbu Y: sumbu Y sekunder dari pemetaan metrik
...
**Tipe:** `"area" | "column" | "areaPercent" | "columnParallel" | "columnPercent" | "line" | "scatter" | undefined`

:::note{title="Deskripsi"}
Tetapkan jenis grafik metrik ini dalam grafik sumbu ganda, hanya berlaku untuk grafik sumbu ganda

\- garis: diagram garis

\- column: Diagram batang

\- columnParallel: Diagram Batang Paralel

\- columnPercent: Persen histogram

\- area: bagan area

\- areaPercent: Bagan persen luas
...
**Tipe:** `boolean | boolean[] | undefined`

:::note{title="Deskripsi"}
Digunakan untuk menentukan apakah skala kedua sumbu diagram sumbu ganda sejajar. Jika terdapat beberapa rangkaian pengukuran, alignTicks dapat dikonfigurasikan sebagai array, dan setiap item sesuai dengan apakah skala diagram sumbu ganda sudah sejajar.
...
**Tipe:** `YLinearAxis | YLinearAxis[] | undefined`

:::note{title="Deskripsi"}
Konfigurasi sumbu Y utama dari bagan sumbu ganda digunakan untuk menentukan sumbu Y utama dari bagan sumbu ganda, termasuk posisi, gaya, dll. dari sumbu Y utama. Jika ada beberapa kelompok ukuran, sumbu Y primer dapat dikonfigurasikan sebagai larik, dan setiap item sesuai dengan sumbu Y utama pada bagan sumbu ganda.
...
:::note{title="Deskripsi"}
Konfigurasi sumbu Y sekunder dari bagan sumbu ganda digunakan untuk menentukan sumbu Y sekunder dari bagan sumbu ganda, termasuk posisi, gaya, dll. dari sumbu Y sekunder. Jika ada beberapa kelompok ukuran, sumbu Y sekunder dapat dikonfigurasikan sebagai larik, setiap item sesuai dengan sumbu Y sekunder dari bagan sumbu ganda.
...
:::note{title="Deskripsi"}
Warna latar belakang bagan. Warna latar belakang dapat berupa string warna, seperti 'merah', 'biru', atau hex, rgb atau rgba'#ff0000', 'rgba(255,0,0,0.5)'
...
:::note{title="Deskripsi"}
Lebar maksimum kolom, yang dapat berupa nilai piksel atau string persentase
...
:::note{title="Deskripsi"}
Dalam kategori yang sama, jarak antar pilar dapat berupa nilai piksel atau string persentase.
...
:::note{title="Deskripsi"}
Gaya primitif persegi panjang



Konfigurasi gaya diagram batang, digunakan untuk menentukan gaya diagram batang pada diagram, termasuk warna diagram batang, batas, sudut membulat, dll.
...
Konfigurasi gaya primitif garis, digunakan untuk menentukan gaya primitif garis pada bagan, termasuk warna, transparansi, kurva, dll. dari garis primitif.
...
:::note{title="Deskripsi"}
Transparansi warna segmen garis
...
:::note{title="Deskripsi"}
Arahkan gaya primitif



Konfigurasi gaya titik primitif digunakan untuk menentukan gaya titik primitif pada bagan, termasuk warna, batas, dll. dari titik primitif.
...
:::note{title="Deskripsi"}
Gaya primitif daerah



Konfigurasi gaya primitif area digunakan untuk menentukan gaya primitif area pada bagan, termasuk warna, transparansi, batas, dll. pada area primitif.
...
:::note{title="Deskripsi"}
Transparansi warna area primitif
...
:::note{title="Deskripsi"}
Konfigurasi internasional, konfigurasi bahasa bagan, mendukung bahasa 'zh\-CN' dan 'en\-US'. Selain itu, Anda dapat memanggil metode intl.setLocale('zh\-CN') untuk mengatur bahasa
