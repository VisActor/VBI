...
Metrik dapat ditampilkan secara berkelompok melalui atribut `parentId` di `measures`.
...
## Perspektif peringkat

Dengan mengonfigurasi properti `encoding` dimensi, Anda dapat mengontrol posisi baris dan kolom dimensi dalam tabel pivot. Dimensi defaultnya ada pada baris, pengaturan `encoding: 'column'` akan menempatkannya pada kolom.
