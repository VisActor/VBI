...
Dengan mengonfigurasi properti `page`, Anda dapat menentukan bahwa diagram menampilkan data untuk nilai dimensi tertentu.

- `field`: field paging.
- `currentValue`: Nilai yang ditampilkan saat ini.
...
Kontrol pemain yang disesuaikan dapat dicapai melalui atribut `page` yang dikombinasikan dengan manajemen status React.
