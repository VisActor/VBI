...
# Render lintas platform

:::tip
`vseed` mendukung serialisasi lossless, jadi `transfer lintas platform`, `salin-tempel` dapat diperoleh langsung melalui `builder` di lingkungan Node dan lingkungan browser.
:::

Contoh berikut menunjukkan cara membuat serialisasi konfigurasi VSeed ke dalam string JSON (mensimulasikan penyalinan), dan cara melakukan deserialisasi dan merender string JSON (mensimulasikan tempel).
