...
Through the `parentId` attribute in `measures`, measures can be displayed in groups.
...
By configuring the `encoding` attribute of the dimension, you can control the row and column position of the dimension in the pivot table. The default dimension is on rows, setting `encoding: 'column'` places it on columns.
