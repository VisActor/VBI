...
:::info
Data and chart types are completely decoupled, and VSeed can always reshape them no matter how many dimensions or metrics there are. The price may be poor semantics for the chart, but it's worth it.
:::

The following example demonstrates the free combination between different structures of `dataset` and different `chartType`. You can click the button below to switch the data set and chart type and observe how VSeed automatically adapts.
