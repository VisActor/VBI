...
:::tip
Normally, chart data processing is time-consuming. VSeed supports split data processing and data rendering. `build` provides `buildAdvanced` and `buildSpec` for separate construction.
:::
...
The following example shows how to use the decoupled build process with the React component.
