...
Boxplots support the following visual channels:
...
Box plots are suitable for displaying data distribution. The X-axis is the category axis (categorical data), the Y-axis is the numerical axis (continuous data), and the boxes are arranged vertically.
...
:::note{title="Description"}
The first dimension of the box plot is mapped to the X-axis, and the remaining dimensions will be merged with the measure name (when there are multiple measures) and displayed as legend items
...
:::note{title="Description"}
All measures of the boxplot will be automatically merged into one measure and mapped to the Y-axis. When there are multiple measures, the measure name will be merged with the other dimensions and displayed as a legend item.
...
\- value: The measure corresponding to the discrete value, used to calculate the statistical value and display the box plot

\- q1: Measure mapping corresponding to the 25th quantile of the statistical value

\- q3: Measure mapping corresponding to the 75th percentile of the statistical value

\- min: measure mapping of box-whisker minimum values

\- max: measure mapping of the maximum value of the box and whiskers

\- meadian: measure mapping of the median statistical value

\- outliers: measure mapping of outliers
...
:::note{title="Description"}
The style configuration of the box plot box supports global or selector granularity to take effect.
...
:::note{title="Description"}
Is the boxPlot primitive visible?
...
:::note{title="Description"}
boxPlot primitive color transparency
...
:::note{title="Description"}
boxPlot primitive border transparency
...
:::note{title="Description"}
Style configuration of exception points, supports global or selector granularity to take effect
...
:::note{title="Description"}
Whisker length configuration for histogram, supports scalar values and arrays of length 2

When the value is a scalar, use whiskers * IQR to calculate the upper and lower bounds

When the value is a 2-element array, whiskers[0] needs to be between [0, 0.25), indicating that the lower bound value takes the corresponding percentile;

whiskers[1] needs to be between (0.75, 1], indicating that the upper bound value takes the corresponding percentile;
...
:::note{title="Description"}
The maximum width of the boxplot can be set as an absolute pixel value or as a percentage (such as '10%')
...
:::note{title="Description"}
The spacing within each group in the grouped box plot can be set to an absolute pixel value or a percentage (such as '10%').
