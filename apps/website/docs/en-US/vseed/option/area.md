...
Area charts are suitable for displaying trends and cumulative relationships of data over time, and enhancing data comparison by filling in areas. The X-axis is the category axis (categorical data), and the Y-axis is the numerical axis (continuous data).

Applicable scenarios:

\- Show trend changes in a single data series

\- Emphasis on the cumulative effect of total amounts over time

\- Compare the total differences in multiple data series
...
\- The first dimension field is mapped to the X-axis, and the remaining dimension fields will be merged with the measure name (when there are multiple measures) and displayed as legend items.

\- All measures will be automatically merged into one measure

Features enabled by default:

\- Module enables stacking

\- Legends, coordinate axes, area fills, data labels, and tooltips are enabled by default.
...
Area chart, a chart type that displays data trends and cumulative relationships.
...
The measures of the area chart will be automatically merged into one measure and mapped to the Y-axis. The measure name will be merged with the other dimensions and displayed as a legend item.
