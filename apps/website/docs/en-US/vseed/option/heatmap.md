...
Heatmaps support the following visual channels:
...
`color`: color channel, supports `one measure`, mapped to color according to measure value
...
Heat map, which displays the distribution and strength relationship of data through the color depth of a two-dimensional matrix

Applicable scenarios:

\- Density and intensity display of large-scale 2D data

\- Correlation analysis between classification and numerical value

\- Cross comparison of time series and categories
...
\- at least 2 dimension fields, used to determine the rows and columns of the heat map

\- at least 1 numeric field (metric), used to map color shades

\- When multiple measures are supported, usually one measure is selected for color mapping

Features enabled by default:

\- Legends, coordinate axes, data labels, tooltips, and numerical scaling are enabled by default.
...
The first dimension of the heat map is mapped to the angle axis, and the remaining dimensions are combined with the measure name (when multiple measures exist) and displayed as legend items.
...
The measures of the heat map will be automatically merged into one measure and mapped to the radius axis. When there are multiple measures, the measure name will be merged with the other dimensions and displayed as a legend item.
...
:::note{title="Description"}
Heat map label configuration is used to define the data labels of the chart and automatically turn on label inversion to ensure label readability.
...
The color legend configuration of the heat map is used to define the legend of the chart, including the location, format, style, etc. of the legend.
...
The tooltips configuration of the heat map is used to define the tooltips of the chart, including the location, format, style, etc. of the tooltips.
