...
`xAxis` : canal de l'axe x, prend en charge `une dimension`, regroupé et calculé par valeur de dimension et affiché sur l'axe x

:::

:::note{title="Description"}
L'histogramme convient aux scénarios montrant la distribution des données. L'axe X est l'axe numérique (données continues), l'axe Y est l'axe numérique (données continues) et les colonnes sont disposées verticalement.

Scénarios applicables :

\- Affichez la distribution des données, telles que la distribution de fréquence, la distribution de probabilité, etc.

\- Analyser la tendance centrale et la dispersion des données

\- Identifiez les valeurs aberrantes et les modèles dans vos données
...
:::note{title="Description"}
Les histogrammes ne nécessitent généralement pas de dimensions
...
:::note{title="Description"}
Les histogrammes ne prennent en charge qu'une seule dimension et les données sont des données discrètes
...
\- value : canal valeur de l'histogramme

\- x0 : canal x0 de l'histogramme

\- x1 : x1 canal d'histogramme
...
:::note{title="Description"}
Numéro de regroupement de l'histogramme, utilisé pour définir le nombre de rectangles de regroupement (colonnes) de l'histogramme.
...
:::note{title="Description"}
La taille du pas de regroupement, utilisée pour calculer la largeur des compartiments, affecte également la largeur des rectangles (colonnes) dans l'histogramme final. Si binCount et binStep sont définis, binStep est prioritaire
...
:::note{title="Description"}
Type de valeur de regroupement d'histogramme, utilisé pour définir le type de valeur de rectangle de regroupement (colonne) de l'histogramme, la valeur par défaut est 'count'
...
:::note{title="Description"}
Les lignes d'étiquettes numériques (valeurs de regroupement) sont affichées dans le sens vertical. La position et le style de la ligne d'étiquette peuvent être définis. Si vous avez besoin de lignes d'étiquette correspondant aux valeurs de regroupement, vous pouvez utiliser cette configuration.
...
:::note{title="Description"}
Configuration de la ligne de régression de densité du noyau, utilisée pour afficher la tendance et la distribution des données
...
:::note{title="Description"}
S'il faut activer la fonction de ligne de régression
