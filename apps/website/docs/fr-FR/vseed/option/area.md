...
Les graphiques en zones conviennent pour afficher les tendances et les relations cumulées des données au fil du temps, ainsi que pour améliorer la comparaison des données en remplissant les zones. L'axe X est l'axe des catégories (données catégorielles) et l'axe Y est l'axe numérique (données continues).

Scénarios applicables :

\- Afficher les changements de tendance dans une seule série de données

\- Accent sur l'effet cumulatif des quantités totales dans le temps

\- Comparez les différences de volume total sur plusieurs séries de données
...
\- Le premier champ de dimension est mappé sur l'axe X et les champs de dimension restants seront fusionnés avec le nom de l'mesure (lorsque plusieurs mesures existent) et affichés sous forme d'éléments de légende.

\- Tous les mesures sont automatiquement fusionnés en un seul mesure

Fonctionnalités activées par défaut :

\- Le module permet l'empilage

\- Les légendes, les axes de coordonnées, les remplissages de zones, les étiquettes de données et les informations d'infobulle sont activés par défaut.
...
Graphique en aires, type de graphique qui affiche les tendances des données et les relations cumulées.
...
Les mesures du graphique en aires seront automatiquement fusionnés en un seul mesure et mappés sur l'axe Y. Le nom de l'mesure sera fusionné avec les autres dimensions et affiché comme élément de légende.
...
Sur la base de deux points de données sélectionnés, tracez une ligne d'étiquette de différence et calculez automatiquement le texte de différence.
