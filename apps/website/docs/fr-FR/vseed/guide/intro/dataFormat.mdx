...
VSeed prend en charge les règles de formatage des valeurs numériques configurées dans `measures`.

- `autoFormat` : s'il faut formater automatiquement.
- `format` : configuration de formatage détaillée, y compris le type, le grossissement, les millièmes, les décimales, les suffixes d'unité, etc.
