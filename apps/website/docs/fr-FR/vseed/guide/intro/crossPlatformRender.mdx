...
:::tip
`vseed` prend en charge la sérialisation sans perte, vous pouvez donc `transmission multiplateforme`, `copier-coller`, et dans l'environnement Node et l'environnement du navigateur, vous pouvez directement obtenir la spécification complète via `builder`
:::

L'exemple suivant montre comment sérialiser une configuration VSeed en une chaîne JSON (simulant une copie), et comment désérialiser et restituer une chaîne JSON (simulant un collage).
