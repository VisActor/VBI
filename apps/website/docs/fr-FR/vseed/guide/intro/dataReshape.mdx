...
:::info
Les types de données et de graphiques sont complètement découplés, et VSeed peut toujours les remodeler, quel que soit le nombre de dimensions ou de métriques. Le prix est peut-être une mauvaise sémantique pour le graphique, mais cela en vaut la peine.
:::

L'exemple ci-dessous démontre la combinaison libre entre différentes structures de `dataset` et différents `chartType`. Vous pouvez cliquer sur le bouton ci-dessous pour changer d'ensemble de données et de type de graphique et observer comment VSeed s'adapte automatiquement.
