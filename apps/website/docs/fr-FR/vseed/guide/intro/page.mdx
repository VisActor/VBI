...
En configurant la propriété `page`, vous pouvez spécifier que le graphique affiche les données pour des valeurs de dimension spécifiques.

- `field` : Champ de pagination.
...
Un contrôle personnalisé du joueur peut être obtenu grâce aux attributs `page` combinés à la gestion de l'état React.
