...
Les mesures peuvent être affichés en groupes via l'attribut `parentId` dans `measures`.
...
## perspective de ligne

En configurant la propriété `encoding` de la dimension, vous pouvez contrôler la position des lignes et des colonnes de la dimension dans le tableau croisé dynamique. La dimension par défaut est sur les lignes, le paramètre `encoding: 'column'` la place sur les colonnes.
