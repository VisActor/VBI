...
Bằng cách định cấu hình thuộc tính `page`, bạn có thể chỉ định rằng biểu đồ hiển thị dữ liệu cho các giá trị thứ nguyên cụ thể.

- `field`: Trường phân trang.
- `currentValue`: Giá trị hiện đang hiển thị.
...
Có thể đạt được khả năng kiểm soát người chơi tùy chỉnh thông qua thuộc tính `page`kết hợp với quản lý trạng thái React.
