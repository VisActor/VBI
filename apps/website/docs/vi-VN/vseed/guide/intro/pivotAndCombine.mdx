...
Thông qua thuộc tính `parentId`trong `measures`, các chỉ số có thể được hiển thị theo nhóm.
...
Bằng cách định cấu hình thuộc tính `encoding`của thứ nguyên, bạn có thể kiểm soát vị trí hàng và cột của thứ nguyên trong bảng tổng hợp. Thứ nguyên mặc định là trên các hàng, cài đặt `encoding: 'column'`sẽ đặt nó trên các cột.
