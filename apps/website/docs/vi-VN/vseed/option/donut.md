...
Biểu đồ bánh rán hỗ trợ các kênh trực quan sau:

`angle`: Kênh góc, hỗ trợ `nhiều chỉ số`, được ánh xạ tới góc cung theo giá trị chỉ số
...
:::note{title="Mô tả"}
Biểu đồ Donut phù hợp để hiển thị mối quan hệ tỷ lệ của dữ liệu một chiều. Có một vùng trống ở giữa để hiển thị thông tin tóm tắt.

Các tình huống áp dụng:

\- Cần hiển thị đồng thời dữ liệu tổng thể và tỷ lệ từng phần

\- Nhấn mạnh mối quan hệ giữa toàn bộ và các bộ phận của dữ liệu

\-Khu vực trung tâm cần hiển thị các chỉ số hoặc tiêu đề chính
...
\- Tất cả các thứ nguyên sẽ được hợp nhất với tên chỉ số (khi có nhiều chỉ số) và hiển thị dưới dạng mục chú giải.

\- Tất cả các chỉ số sẽ được tự động hợp nhất thành một chỉ số

Các tính năng được bật theo mặc định:

\-Chú giải, nhãn dữ liệu, thông tin nhắc nhở, tính toán tỷ lệ và văn bản ở giữa được bật theo mặc định.
...
Biểu đồ Donut, biến thể của biểu đồ hình tròn với khoảng trắng ở giữa
...
Một tập dữ liệu tổng hợp tuân thủ đặc tả TidyData được dùng để xác định nguồn dữ liệu và cấu trúc của biểu đồ. Tập dữ liệu do người dùng nhập vào không yêu cầu bất kỳ xử lý nào. VSeed có chức năng định hình lại dữ liệu mạnh mẽ và sẽ tự định hình lại dữ liệu. Dữ liệu biểu đồ bánh rán cuối cùng sẽ được chuyển đổi thành 1 thứ nguyên và 1 chỉ số.
...
Tất cả các kích thước của biểu đồ vành khuyên sẽ được kết hợp với tên chỉ số (khi có nhiều chỉ số) thành một thứ nguyên, ánh xạ tới góc của biểu đồ hình tròn và hiển thị dưới dạng mục chú giải.
...
Tất cả các chỉ số của biểu đồ vành khuyên sẽ tự động được hợp nhất thành một chỉ số, ánh xạ tới bán kính của biểu đồ hình tròn. Khi có nhiều chỉ số, tên chỉ số sẽ được hợp nhất với các kích thước còn lại và hiển thị dưới dạng mục chú giải.
