...
\- cấu hình trường khuyến nghị: `2`chỉ số, `2`chiều
...
Biểu đồ trục kép hỗ trợ các kênh trực quan sau:

`xAxis`: kênh trục X, hỗ trợ `nhiều chiều`, ánh xạ tới trục X theo giá trị thứ nguyên

`primaryYAxis`: Kênh trục Y trục chính, hỗ trợ `nhiều chỉ số`, ánh xạ các chỉ số tới trục chính

`secondaryYAxis`: Kênh trục Y trục phụ, hỗ trợ `nhiều chỉ số`, ánh xạ chỉ số tới trục phụ
...
`tooltip`: Kênh nhắc nhở, hỗ trợ `nhiều chiều`và `nhiều chỉ số`, sẽ hiển thị khi di chuột qua điểm dữ liệu
...
:::note{title="Mô tả"}
Biểu đồ hai trục thích hợp để thể hiện mối quan hệ so sánh giữa hai chỉ số có độ lớn hoặc đơn vị khác nhau, bao gồm trục tọa độ chính và trục tọa độ phụ.

Các tình huống áp dụng:

\- Phân tích so sánh các chỉ số có độ lớn khác nhau

\- So sánh xu hướng của các chỉ số tương quan

\- Các chỉ số tổng hợp như giá trị số và tốc độ tăng trưởng cần được hiển thị cùng lúc

\-Hỗ trợ các loại kết hợp biểu đồ khác nhau (chẳng hạn như biểu đồ đường + biểu đồ thanh/biểu đồ đường+biểu đồ vùng/biểu đồ vùng+biểu đồ thanh)
...
\-Hỗ trợ các nhóm chỉ số, nhóm chỉ số đầu tiên sẽ được đặt trên trục bên trái (chính), và nhóm chỉ số thứ hai sẽ được đặt trên trục phải (phụ)
...
\- Hai bộ trường chỉ số có thể được ánh xạ tương ứng sang trục Y bên trái và bên phải và tất cả các chỉ số trong một nhóm chỉ số sẽ được tự động hợp nhất thành một chỉ số.

Các tính năng được bật theo mặc định:

\- Trục, chú giải, nhãn dữ liệu và thông tin nhắc nhở được bật theo mặc định.
...
:::note{title="Mô tả"}
Biểu đồ trục kép, biểu đồ tổng hợp thể hiện mối quan hệ so sánh giữa hai chỉ số có cường độ khác nhau
...
:::note{title="Mô tả"}
Tập dữ liệu, một tập dữ liệu tuân thủ các thông số kỹ thuật của TidyData và đã được tổng hợp, được sử dụng để xác định nguồn dữ liệu và cấu trúc của biểu đồ. Tập dữ liệu do người dùng nhập vào không yêu cầu bất kỳ xử lý nào. VSeed có chức năng định hình lại dữ liệu mạnh mẽ và sẽ tự định hình lại dữ liệu. Dữ liệu của biểu đồ trục kép cuối cùng sẽ được chuyển đổi thành 2 chiều, 1 hoặc 2 chỉ số (tùy thuộc vào việc người dùng đã cấu hình nhóm chỉ số hay chưa).

:::

**Ví dụ**
```ts
[{tháng:'Tháng Một', giá trị:100, mức tăng trưởng:0,2}, {tháng:'Tháng Hai', giá trị:150, mức tăng trưởng:0,5}]
...
:::note{title="Mô tả"}
Thứ nguyên, thứ nguyên đầu tiên sẽ được đặt trên trục X và các thứ nguyên còn lại sẽ được hợp nhất với tên chỉ số (khi có nhiều chỉ số) và hiển thị dưới dạng mục chú giải.

:::

**Ví dụ**
```ts
[{id: 'tháng', bí danh: 'tháng'}]
...
:::note{title="Mô tả"}
Chỉ số biểu đồ trục kép

Đối với các chỉ số được ánh xạ tới trục Y chính và trục Y phụ trong mã hóa,

Bạn có thể nhóm các chỉ số bằng cách đặt thuộc tính `parentId`và các chỉ số trong các nhóm khác nhau sẽ được hiển thị trong các biểu đồ con khác nhau.

Bạn cũng có thể đặt thuộc tính `chartType`để chỉ định loại biểu đồ cho các nhóm chỉ số khác nhau.
...
\- PrimaryYAxis: trục Y chính của ánh xạ chỉ số

\- trục Y thứ cấp: trục Y thứ cấp của ánh xạ chỉ số
...
:::note{title="Mô tả"}
Đặt loại biểu đồ của chỉ số này trong biểu đồ trục kép, chỉ áp dụng cho biểu đồ trục kép

\- line: biểu đồ đường

\- cột: biểu đồ cột

\- columnsParallel: biểu đồ cột song song

\-columnPercent: biểu đồ cột phần trăm

\- khu vực: biểu đồ khu vực

\- AreaPercent: biểu đồ diện tích phần trăm
...
:::note{title="Mô tả"}
Được sử dụng để xác định xem tỷ lệ của hai trục của biểu đồ trục kép có được căn chỉnh hay không. Khi có nhiều bộ thước đo, AlignTicks có thể được định cấu hình dưới dạng một mảng và mỗi mục tương ứng với việc tỷ lệ của biểu đồ hai trục có được căn chỉnh hay không.
...
:::note{title="Mô tả"}
Cấu hình trục Y chính của biểu đồ trục kép được sử dụng để xác định trục Y chính của biểu đồ trục kép, bao gồm vị trí, kiểu, v.v. của trục Y chính. Khi có nhiều nhóm thước đo, trục Y chính có thể được đặt cấu hình dưới dạng một mảng và mỗi mục tương ứng với trục Y chính của biểu đồ trục kép.
...
:::note{title="Mô tả"}
Cấu hình trục Y phụ của biểu đồ trục kép được sử dụng để xác định trục Y phụ của biểu đồ trục kép, bao gồm vị trí, kiểu, v.v. của trục Y phụ. Khi có nhiều nhóm thước đo, trục Y phụ có thể được định cấu hình dưới dạng một mảng, mỗi mục tương ứng với trục Y phụ của biểu đồ hai trục.
...
:::note{title="Mô tả"}
Màu nền của biểu đồ. Màu nền có thể là một chuỗi màu, chẳng hạn như 'đỏ', 'xanh' hoặc hex, rgb hoặc rgba'#ff0000', 'rgba(255,0,0,0.5)'
...
:::note{title="Mô tả"}
Trong cùng một danh mục, khoảng cách giữa các cột có thể là giá trị pixel hoặc chuỗi phần trăm.
...
:::note{title="Mô tả"}
Phong cách nguyên thủy hình chữ nhật



Cấu hình kiểu biểu đồ thanh, dùng để xác định kiểu biểu đồ thanh của biểu đồ, bao gồm màu biểu đồ thanh, đường viền, các góc tròn, v.v.
...
Cấu hình kiểu nguyên thủy của đường, được sử dụng để xác định kiểu nguyên thủy của biểu đồ, bao gồm màu sắc, độ trong suốt, đường cong, v.v. của đường nguyên thủy.
...
Cấu hình kiểu gốc điểm được sử dụng để xác định kiểu gốc điểm của biểu đồ, bao gồm màu sắc, đường viền, v.v. của kiểu gốc điểm.
...
:::note{title="Mô tả"}
Kiểu nguyên thủy diện tích



Cấu hình kiểu nguyên thủy của vùng được sử dụng để xác định kiểu nguyên thủy của vùng của biểu đồ, bao gồm màu sắc, độ trong suốt, đường viền, v.v. của vùng nguyên thủy.
...
:::note{title="Mô tả"}
Liệu khu vực nguyên thủy có thể nhìn thấy được hay không
...
:::note{title="Mô tả"}
Độ trong suốt của màu của vùng nguyên thủy
...
:::note{title="Mô tả"}
Cấu hình quốc tế hóa, cấu hình ngôn ngữ biểu đồ, hỗ trợ hai ngôn ngữ 'zh\-CN' và 'en\-US', ngoài ra bạn có thể gọi phương thức intl.setLocale('zh\-CN') để thiết lập ngôn ngữ
