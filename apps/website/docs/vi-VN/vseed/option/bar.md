...
:::note{title="Mô tả"}
Biểu đồ thanh, phù hợp với các kịch bản so sánh dữ liệu theo chiều ngang. trục Y là trục danh mục (dữ liệu phân loại), trục X là trục số (dữ liệu liên tục) và các cột được sắp xếp theo chiều ngang.

Các tình huống áp dụng:

\- Khi tên mục dữ liệu dài

\- Cần hiển thị dữ liệu so sánh xếp hạng

\- Hiển thị dữ liệu hai chiều tích cực và tiêu cực
...
:::note{title="Mô tả"}
Nguồn dữ liệu, một tập dữ liệu tuân thủ các thông số kỹ thuật của TidyData và đã được tổng hợp, được sử dụng để xác định nguồn dữ liệu và cấu trúc của biểu đồ. Tập dữ liệu do người dùng nhập vào không yêu cầu bất kỳ xử lý nào. VSeed có chức năng định hình lại dữ liệu mạnh mẽ và sẽ tự định hình lại dữ liệu. Dữ liệu biểu đồ thanh cuối cùng sẽ được chuyển đổi thành 2 chiều và 1 chỉ số.
...
**Ví dụ**
```ts
[{id: "ngày", bí danh: "ngày"}, {id: "value", bí danh: "value"}]
...
Các chỉ số và chỉ số biểu đồ thanh sẽ được tự động hợp nhất thành một chỉ số và ánh xạ tới trục X. Khi có nhiều chỉ số, tên chỉ số sẽ được hợp nhất với các thứ nguyên khác và hiển thị dưới dạng mục chú giải.
