...
Boxplots hỗ trợ các kênh hình ảnh sau:
...
:::note{title="Mô tả"}
Biểu đồ hình hộp phù hợp để hiển thị phân phối dữ liệu. trục X là trục danh mục (dữ liệu phân loại), trục Y là trục số (dữ liệu liên tục) và các hộp được sắp xếp theo chiều dọc.

Các tình huống áp dụng:

\- khi tên mục dữ liệu ngắn

\- Cần so sánh trực quan kích thước số của các loại khác nhau

\- Hiển thị xu hướng của dữ liệu chuỗi thời gian
...
:::note{title="Mô tả"}
Kích thước đầu tiên của ô hộp được ánh xạ tới trục X và các kích thước còn lại sẽ được hợp nhất với tên chỉ số (khi có nhiều chỉ số) và hiển thị dưới dạng mục chú giải
...
:::note{title="Mô tả"}
Tất cả các chỉ số của boxplot sẽ được tự động hợp nhất thành một chỉ số và ánh xạ tới trục Y. Khi có nhiều chỉ số, tên chỉ số sẽ được hợp nhất với các thứ nguyên khác và hiển thị dưới dạng mục chú giải.
...
\- value: Chỉ số tương ứng với giá trị rời rạc, dùng để tính giá trị thống kê và hiển thị ô đồ thị

\- q1: Ánh xạ chỉ số tương ứng với phân vị thứ 25 của giá trị thống kê

\- q3: Ánh xạ chỉ số tương ứng với phân vị thứ 75 của giá trị thống kê

\- min: ánh xạ chỉ số của các giá trị tối thiểu của râu hộp

\- max: ánh xạ chỉ số giá trị tối đa của hộp và râu

\- meadian: ánh xạ chỉ số của giá trị thống kê trung bình

\- ngoại lệ: ánh xạ chỉ số của các ngoại lệ
...
:::note{title="Mô tả"}
Cấu hình kiểu của hộp vẽ hộp hỗ trợ độ chi tiết toàn cục hoặc bộ chọn để có hiệu lực.
...
:::note{title="Mô tả"}
Có thể nhìn thấy nguyên thủy boxPlot không?
...
:::note{title="Mô tả"}
boxPlot màu trong suốt nguyên thủy
...
:::note{title="Mô tả"}
Màu đường viền phần tử boxPlot
...
:::note{title="Mô tả"}
chiều rộng đường viền phần tử boxPlot
...
:::note{title="Mô tả"}
boxPlot trong suốt đường viền nguyên thủy
...
:::note{title="Mô tả"}
Cấu hình kiểu của các điểm ngoại lệ, hỗ trợ mức độ chi tiết toàn cục hoặc bộ chọn để có hiệu lực
...
:::note{title="Mô tả"}
Cấu hình độ dài râu cho biểu đồ, hỗ trợ các giá trị vô hướng và mảng có độ dài 2

Khi giá trị là vô hướng, hãy sử dụng râu * IQR để tính giới hạn trên và giới hạn dưới

Khi giá trị là mảng 2 phần tử, râu [0] cần nằm trong khoảng [0, 0,25), biểu thị rằng giá trị giới hạn dưới chiếm phần trăm tương ứng;

râu [1] cần nằm trong khoảng (0,75, 1], biểu thị rằng giá trị giới hạn trên chiếm phần trăm tương ứng;
...
:::note{title="Mô tả"}
Độ rộng tối đa của boxplot có thể được đặt dưới dạng giá trị pixel tuyệt đối hoặc dưới dạng phần trăm (chẳng hạn như '10%')
...
:::note{title="Mô tả"}
Khoảng cách trong mỗi nhóm trong ô hộp được nhóm có thể được đặt thành giá trị pixel tuyệt đối hoặc phần trăm (chẳng hạn như '10%').
