...
:::note{title="Mô tả"}
Biểu đồ thanh tỷ lệ phần trăm, phù hợp với các tình huống trong đó tỷ lệ của từng danh mục được hiển thị theo chiều ngang. trục X hiển thị tỷ lệ dữ liệu dưới dạng phần trăm.

Các tình huống áp dụng:

\- So sánh tỷ lệ khi tên danh mục dài

\- Phân tích thành phần dữ liệu đa chiều theo chiều ngang

\- Kịch bản hiển thị đồng thời thứ hạng và tỷ lệ
...
\- ít nhất 1 trường chiều và 1 trường đo

\-Tổng tỷ lệ của tất cả các loại là 100%

\-Hỗ trợ xếp chồng nhiều chuỗi để hiển thị các mối quan hệ tỷ lệ
...
:::note{title="Mô tả"}
Biểu đồ thanh tỷ lệ phần trăm, thể hiện mối quan hệ giữa tỷ lệ của từng loại dữ liệu dưới dạng tỷ lệ phần trăm theo chiều ngang
...
:::note{title="Mô tả"}
Nguồn dữ liệu, một tập dữ liệu tuân theo các thông số kỹ thuật của TidyData và đã được tổng hợp, được sử dụng để xác định nguồn dữ liệu và cấu trúc của biểu đồ. Tập dữ liệu do người dùng nhập vào không yêu cầu bất kỳ xử lý nào. VSeed có chức năng định hình lại dữ liệu mạnh mẽ và sẽ tự định hình lại dữ liệu. Dữ liệu của biểu đồ thanh phần trăm cuối cùng sẽ được chuyển đổi thành 2 chiều và 1 chỉ số.
...
:::note{title="Mô tả"}
Thứ nguyên, thứ nguyên đầu tiên sẽ được đặt trên trục Y và các thứ nguyên còn lại sẽ được hợp nhất với tên chỉ số (khi có nhiều chỉ số) và hiển thị dưới dạng mục chú giải.
...
:::note{title="Mô tả"}
Các chỉ số sẽ được tự động hợp nhất thành một chỉ số và ánh xạ tới trục X. Khi có nhiều chỉ số, tên chỉ số sẽ được hợp nhất với các thứ nguyên khác và hiển thị dưới dạng mục chú giải.
