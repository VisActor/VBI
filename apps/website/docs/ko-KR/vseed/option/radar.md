...
:::info{title="인코딩 매핑"}
레이더 차트는 다음 시각적 채널을 지원합니다:

`angle`  : 각도 채널, `여러 차원`을 지원하며, 차원 값에 따라 각도 축에 매핑

`radius` : 반경 채널, `여러 지표`를 지원하며, 지표 값에 따라 반경 축에 매핑
...
:::note{title="설명"}
레이더 차트, 다차원 데이터의 비교 분석에 적합하며, 다축 좌표계를 통해 각 차원의 수치 분포를 표시

적용 시나리오:

\- 다차원 데이터의 종합 성능 비교

\- 여러 객체의 여러 지표에 대한 성능 평가
...
\- 첫 번째 차원은 레이더 차트의 각 차원 축으로, 다른 차원은 서로 다른 시리즈로 비교

\- 여러 지표를 각각 다른 시리즈로 표시 지원

기본 활성화 기능:

\- 기본적으로 범례, 레이더 좌표계, 데이터 레이블, 툴팁, 수치 확대/축소 활성화
...
레이더 차트, 다축 좌표계를 통해 다차원 데이터 비교 관계 표시
...
TidyData 규격을 준수하고 이미 집계된 데이터셋으로, 차트의 데이터 소스와 구조를 정의하는 데 사용됩니다. 사용자가 입력한 데이터셋은 어떤 처리도 필요하지 않으며, VSeed는 강력한 데이터 재구성 기능을 통해 자체적으로 데이터를 재구성합니다. 장미 그래프의 데이터는 최종적으로 2개의 차원, 1개의 지표로 변환됩니다.
...
레이더 차트의 첫 번째 차원은 각도 축에 매핑되고, 나머지 차원은 지표 이름(여러 지표가 있는 경우)과 병합되어 범례 항목으로 표시됩니다.
...
레이더 차트의 지표는 자동으로 하나의 지표로 병합되어 반경 축에 매핑되며, 여러 지표가 있는 경우 지표 이름이 나머지 차원과 병합되어 범례 항목으로 표시됩니다.
...
:::note{title="설명"}
레이더 차트 진입 효과, 방사형 및 확대/축소 애니메이션 지원
