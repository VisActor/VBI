...
:::note{title="설명"}
백분율 막대 차트는 각 범주의 비율 관계를 표시하는 시나리오에 적합하며, Y축은 백분율 형태로 데이터 비율을 표시합니다
...
백분율 막대 차트는 백분율 형태로 각 범주 데이터의 비율 관계를 표시합니다
...
TidyData 규격에 맞고 이미 집계된 데이터셋으로, 차트의 데이터 출처와 구조를 정의하는 데 사용됩니다. 사용자가 입력한 데이터셋은 별도의 처리가 필요하지 않으며, VSeed에 강력한 데이터 재구성 기능이 내장되어 자체적으로 데이터를 재구성합니다. 백분율 막대 차트의 데이터는 최종적으로 2개의 차원, 1개의 지표로 변환됩니다.
...
백분율 막대 차트의 지표는 자동으로 하나의 지표로 병합되어 Y축에 매핑되며, 여러 지표가 있는 경우 지표 이름이 나머지 차원과 병합되어 범례 항목으로 표시됩니다.
...
:::note{title="설명"}
사각형 그래픽 요소 스타일로, 차트의 사각형 그래픽 요소 스타일을 정의하며 색상, 테두리, 둥근 모서리 등을 포함합니다.
