...
:::note{title="설명"}
막대 차트는 세로 방향 데이터 비교 시나리오에 적합하며, X축은 범주 축(분류 데이터), Y축은 값 축(연속 데이터)이며 막대가 세로로 정렬됩니다
...
:::note{title="설명"}
막대 차트의 첫 번째 차원은 X축에 매핑되고, 나머지 차원은 지표 이름(여러 지표가 있는 경우)과 병합되어 범례 항목으로 표시됩니다
...
:::note{title="설명"}
막대 차트의 모든 지표는 자동으로 하나의 지표로 병합되어 Y축에 매핑되며, 여러 지표가 있는 경우 지표 이름이 나머지 차원과 병합되어 범례 항목으로 표시됩니다.
