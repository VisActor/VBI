...
:::note{title="설명"}
그룹 막대 차트는 여러 지표를 병렬로 비교하는 시나리오에 적합하며, 여러 막대가 나란히 배열되어 서로 다른 지표 값을 표시합니다

적용 시나리오:

\- 동일 차원 내 여러 지표의 병렬 비교
...
\- 기본적으로 범례, 축, 데이터 레이블, 툴팁, 지표 정렬이 활성화됩니다
...
:::note{title="설명"}
그룹 막대 차트는 여러 지표를 병렬로 비교하는 시나리오에 적합합니다
...
:::note{title="설명"}
데이터셋, TidyData 규격에 맞고 이미 집계된 데이터셋으로, 차트의 데이터 출처와 구조를 정의하는 데 사용됩니다. 사용자가 입력한 데이터셋은 별도의 처리가 필요하지 않으며, VSeed에 강력한 데이터 재구성 기능이 내장되어 자체적으로 데이터를 재구성합니다. 그룹 막대 차트의 데이터는 최종적으로 2개의 차원, 1개의 지표로 변환됩니다.
...
:::note{title="설명"}
차원, 첫 번째 차원은 X축에 매핑되고, 나머지 차원은 지표 이름(여러 지표가 있는 경우)과 병합되어 범례 항목으로 표시됩니다.
...
:::note{title="설명"}
지표, 그룹 막대 차트의 모든 지표는 자동으로 하나의 지표로 병합되어 Y축에 매핑되며, 여러 지표가 있는 경우 지표 이름이 나머지 차원과 병합되어 범례 항목으로 표시됩니다.
...
:::note{title="설명"}
사각형 그래픽 요소 스타일, 그룹 막대 차트 스타일 구성으로, 그룹 막대 차트의 색상, 테두리, 둥근 모서리 등을 정의하는 데 사용됩니다.
