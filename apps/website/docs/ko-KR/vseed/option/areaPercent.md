...
:::note{title="설명"}
퍼센트 영역 차트는 시간에 따른 여러 카테고리의 비율 변화 추세를 표시하는 데 적합하며, Y축은 백분율 형태로 비율 관계를 보여줍니다.
...
퍼센트 영역 차트는 특정 차원에 따른 여러 카테고리 비율의 변화를 백분율 형태로 표시합니다.
...
TidyData 규격을 따르고 이미 집계된 데이터셋으로, 차트의 데이터 소스와 구조를 정의하는 데 사용됩니다. 사용자가 입력한 데이터셋은 별도의 처리가 필요하지 않으며, VSeed는 강력한 데이터 재구성 기능을 통해 자체적으로 데이터를 재구성합니다. 퍼센트 영역 차트의 데이터는 최종적으로 2개의 차원, 1개의 지표로 변환됩니다.

:::

**예시**
```ts
[{month:'1월', category:'A', value:30}, {month:'1월', category:'B', value:70}]
...
퍼센트 영역 차트의 지표는 자동으로 하나의 지표로 병합되어 Y축에 매핑되며, 지표 이름은 나머지 차원과 병합되어 범례 항목으로 표시됩니다.
