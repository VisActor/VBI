...
:::info{title="인코딩 매핑"}
퍼널 차트는 다음 시각적 채널을 지원합니다:

`size`   : 크기 채널, `여러 지표`를 지원하며, 지표 값에 따라 퍼널 너비에 매핑됩니다.
...
:::note{title="설명"}
퍼널 차트, 단일 차원 데이터의 비율 관계를 표시하는 데 사용됩니다.

적용 시나리오:

퍼널 차트 적용 시나리오:

\- 여러 연속적이고 정규화된 단계가 있는 프로세스를 분석하고 각 단계에서의 데이터 이탈 또는 전환 상황을 명확하게 표시하는 데 적합합니다.
...
퍼널 차트의 모든 차원은 지표 이름(여러 지표가 있는 경우)과 병합되어 하나의 차원이 되며, 범례 항목으로 표시됩니다.
...
퍼널 차트의 모든 지표는 자동으로 하나의 지표로 병합되며, 여러 지표가 있는 경우 지표 이름이 나머지 차원과 병합되어 범례 항목으로 표시됩니다.
...
:::note{title="설명"}
선형 그래디언트 색상 구성, 차트의 색상 방식을 정의하는 데 사용됩니다.
...
:::note{title="설명"}
색상 범례 구성, 차트의 범례를 정의하는 데 사용되며, 범례의 위치, 형식, 스타일 등을 포함합니다.
