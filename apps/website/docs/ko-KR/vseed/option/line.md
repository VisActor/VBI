...
:::info{title="인코딩 매핑"}
꺾은선형 차트는 다음 시각적 채널을 지원합니다:

`x`      : X축 채널, `여러 차원`을 지원하며, 차원 값에 따라 X축에 매핑됩니다.

`y`      : Y축 채널, `여러 지표`를 지원하며, 지표 값에 따라 Y축에 매핑됩니다.
...
:::note{title="설명"}
꺾은선형 차트, 시간 또는 정렬된 카테고리에 따른 데이터 변화 추세를 표시하는 데 적합하며, 선분으로 데이터 포인트를 연결하여 추세선을 형성합니다.
...
:::note{title="설명"}
꺾은선형 차트, 시간 또는 정렬된 카테고리에 따른 데이터 변화 추세를 표시하는 데 적합합니다.
...
:::note{title="설명"}
데이터 소스, TidyData 규격을 준수하고 이미 집계된 데이터셋으로, 차트의 데이터 소스와 구조를 정의하는 데 사용됩니다. 사용자가 입력한 데이터셋은 별도의 처리가 필요하지 않으며, VSeed는 강력한 데이터 리쉐이핑 기능을 갖추고 있어 자체적으로 데이터를 재구성합니다. 꺾은선형 차트의 데이터는 최종적으로 2개의 차원, 1개의 지표로 변환됩니다.
...
:::note{title="설명"}
차원, 꺾은선형 차트의 첫 번째 차원은 X축에 매핑되고, 나머지 차원은 지표 이름(여러 지표가 있는 경우)과 병합되어 범례 항목으로 표시됩니다.
...
:::note{title="설명"}
지표, 꺾은선형 차트의 모든 지표는 자동으로 하나의 지표로 병합되어 Y축에 매핑되며, 여러 지표가 있는 경우 지표 이름이 나머지 차원과 병합되어 범례 항목으로 표시됩니다.
