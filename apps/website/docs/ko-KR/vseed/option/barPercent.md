...
:::note{title="설명"}
백분율 막대 차트, 각 카테고리의 비율 관계를 가로로 표시하는 시나리오에 적합하며, X축이 백분율 형태로 데이터 비율을 표시합니다.

적용 시나리오:

\- 카테고리 이름이 긴 경우의 비율 비교

\- 다차원 데이터의 가로 구성 분석

\- 순위와 비율을 동시에 표시하는 시나리오
...
\- 최소 1개의 차원 필드와 1개의 측정 필드

\- 모든 카테고리 비율의 합이 100%

\- 여러 시리즈를 누적하여 비율 관계 표시 지원

기본 활성화 기능:

\- 기본적으로 범례, 좌표축, 백분율 레이블, 툴팁, 비율 계산이 활성화됩니다.
...
:::note{title="설명"}
백분율 막대 차트, 가로 백분율 형태로 각 카테고리 데이터의 비율 관계를 표시합니다.
...
:::note{title="설명"}
데이터 소스, TidyData 사양을 따르고 이미 집계된 데이터 세트로, 차트의 데이터 출처와 구조를 정의하는 데 사용됩니다. 사용자가 입력한 데이터 세트는 어떠한 처리도 필요하지 않으며, VSeed는 강력한 데이터 재구성 기능을 갖추고 있어 자체적으로 데이터를 재구성합니다. 백분율 막대 차트의 데이터는 최종적으로 2개의 차원, 1개의 측정값으로 변환됩니다.
...
:::note{title="설명"}
차원, 첫 번째 차원은 Y축에 배치되고, 나머지 차원은 측정값 이름(여러 측정값이 있는 경우)과 병합되어 범례 항목으로 표시됩니다.
...
:::note{title="설명"}
측정값, 측정값은 자동으로 하나의 측정값으로 병합되어 X축에 매핑됩니다. 여러 측정값이 있는 경우 측정값 이름은 나머지 차원과 병합되어 범례 항목으로 표시됩니다.
