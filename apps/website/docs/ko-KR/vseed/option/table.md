...
:::info{title="권장"}
\- 권장 필드 구성: `임의`개 측정값, `임의`개 차원

\- 데이터 재구성 지원: 최소 `임의`개 측정값, `임의`개 차원

:::

:::info{title="인코딩 매핑"}
차원 트리와 측정값 트리만 구성 가능, 기본적으로 encoding은 column으로 설정

:::

:::note{title="설명"}
테이블, 상세 데이터 표시에 적합, 행과 열이 명확하여 특정 값 확인이 용이
...
\- 차원 필드는 테이블의 열 제목으로 사용

기본 활성화 기능:

\- 정렬, 필터링, 페이지네이션 기능 기본 활성화
...
:::note{title="설명"}
TidyData 규격을 준수하고 이미 집계된 데이터셋으로, 차트의 데이터 출처와 구조를 정의하는 데 사용, 사용자가 입력한 데이터셋은 별도의 처리가 필요하지 않음, 하나의 필드는 하나의 열에 대응, 하나의 레코드는 하나의 행에 대응
...
:::note{title="설명"}
테이블의 각 측정값은 하나의 행에 대응하며, 기본적으로 측정값 조합을 지원함.
...
:::note{title="설명"}
측정값 그룹 별칭, 중복 허용, 미입력 시 alias는 id와 동일
...
:::note{title="설명"}
마우스를 열 헤더 셀에 올렸을 때의 배경 색상, 마우스가 위치한 셀을 강조 표시하는 데 사용
...
:::note{title="설명"}
마우스를 열 헤더에 올렸을 때 전체 행 셀의 배경 색상, 마우스가 위치한 행을 강조 표시하는 데 사용
