...
:::note{title="설명"}
원형 패킹 차트, 계층 데이터를 표시하는 데 사용되며, 원의 크기로 값의 크기를 나타냅니다.
...
\- 최소 1개의 숫자 필드(측정값)가 원의 크기 매핑에 필요
