...
데이터가 시간에 따라 변하는 순위를 표시하는 데 적합, 막대는 세로로 배열

적용 시나리오:

\- 데이터 항목 이름이 긴 경우

\- 다양한 카테고리의 값을 직관적으로 비교하고 시간에 따른 정렬 변화를 표시해야 하는 경우
...
\- X축은 카테고리 축(분류 데이터)으로 차원 값을 표시

\- Y축은 값 축(연속 데이터)으로 지표 값을 표시

\- 플레이어를 통해 시간 차원을 제어하고 데이터 변화를 동적으로 표시 지원

\- 막대가 애니메이션 중 값 크기에 따라 동적으로 정렬
...
:::note{title="설명"}
동적 세로 막대 차트, 데이터가 시간에 따라 변하는 순위를 표시하는 데 적합
...
첫 번째 차원은 player에 매핑, 두 번째 차원은 X축에 매핑
...
동적 세로 막대 차트의 모든 지표는 자동으로 하나의 지표로 병합되어 Y축에 매핑되며, 여러 지표가 있을 경우 지표 이름이 나머지 차원과 병합되어 범례 항목으로 표시됩니다.
...
:::note{title="설명"}
플레이어 설정, 시간 차원을 지정하는 데 사용되며 동적 세로 막대 차트의 핵심 설정

플레이어를 통해 시간 차원의 재생 진행을 제어하여 데이터의 동적 업데이트 및 정렬 변화 구현
...
:::note{title="설명"}
정렬 설정, 동적 세로 막대 차트는 일반적으로 값에 따라 동적으로 정렬해야 함
...
:::note{title="설명"}
페이지 설정, 데이터 양이 많은 시나리오를 처리하는 데 사용
...
:::note{title="설명"}
툴팁 설정, 마우스 호버 시 상세 정보를 표시하는 데 사용
...
:::note{title="설명"}
영역 선택 설정, 영역 선택 상호작용을 지원하는 데 사용
...
:::note{title="설명"}
X축 설정, 카테고리 축으로 차원 값을 표시, 막대는 세로로 배열
...
:::note{title="설명"}
막대 스타일 설정, 단일 스타일 또는 배열 형식으로 가능
...
:::note{title="설명"}
마커 포인트 설정, 특정 데이터 포인트에 표시를 추가하는 데 사용
...
:::note{title="설명"}
차원 값 표시선, 가로 표시선, 특정 Y축 카테고리를 표시
...
:::note{title="설명"}
차원 연동 설정, 여러 차트 간의 차원 연동 상호작용 지원
