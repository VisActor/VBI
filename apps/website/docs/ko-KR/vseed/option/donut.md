...
:::note{title="설명"}
도넛 차트는 단일 차원 데이터의 비율 관계를 표시하는 데 적합하며, 중앙에 빈 영역을 두어 요약 정보를 표시할 수 있습니다
...
\- 모든 차원은 지표 이름(여러 지표가 있는 경우)과 병합되어 범례 항목으로 표시됩니다.

\- 모든 측정값은 자동으로 하나의 측정값으로 병합됩니다.

기본 활성화 기능:

\- 기본적으로 범례, 데이터 레이블, 툴팁, 비율 계산, 중앙 텍스트가 활성화됩니다
...
도넛 차트는 중앙에 빈 영역이 있는 파이 차트의 변형입니다
...
TidyData 규격에 맞고 이미 집계된 데이터셋으로, 차트의 데이터 출처와 구조를 정의하는 데 사용됩니다. 사용자가 입력한 데이터셋은 별도의 처리가 필요하지 않으며, VSeed에 강력한 데이터 재구성 기능이 내장되어 자체적으로 데이터를 재구성합니다. 도넛 차트의 데이터는 최종적으로 1개의 차원, 1개의 지표로 변환됩니다.
...
도넛 차트의 모든 차원은 지표 이름(여러 지표가 있는 경우)과 병합되어 1개의 차원이 되고, 파이 차트의 각도에 매핑되며 범례 항목으로 표시됩니다.
...
도넛 차트의 모든 지표는 자동으로 하나의 지표로 병합되어 파이 차트의 반지름에 매핑되며, 여러 지표가 있는 경우 지표 이름이 나머지 차원과 병합되어 범례 항목으로 표시됩니다.
