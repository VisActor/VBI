...
:::note{title="설명"}
병렬 막대 차트, 여러 측정값을 가로로 병렬 비교하는 시나리오에 적합하며, 여러 막대가 평행하게 배열되어 서로 다른 측정값을 표시합니다.

적용 시나리오:

\- 카테고리 이름이 긴 경우의 다중 측정값 비교

\- 순위와 값을 동시에 표시하는 가로 비교
...
\- 첫 번째 차원은 Y축에 배치되고, 나머지 차원은 지표 이름(여러 지표가 있는 경우)과 병합되어 범례 항목으로 표시됩니다.
...
:::note{title="설명"}
병렬 막대 차트, 여러 측정값을 가로로 병렬 비교하는 시나리오에 적합합니다.
...
:::note{title="설명"}
데이터 소스, TidyData 사양을 따르고 이미 집계된 데이터 세트로, 차트의 데이터 출처와 구조를 정의하는 데 사용됩니다. 사용자가 입력한 데이터 세트는 어떠한 처리도 필요하지 않으며, VSeed는 강력한 데이터 재구성 기능을 갖추고 있어 자체적으로 데이터를 재구성합니다. 병렬 막대 차트의 데이터는 최종적으로 2개의 차원, 1개의 측정값으로 변환됩니다.
...
:::note{title="설명"}
차원, 첫 번째 차원은 Y축에 매핑되고, 나머지 차원은 지표 이름(여러 지표가 있는 경우)과 병합되어 범례 항목으로 표시됩니다.
...
:::note{title="설명"}
측정값, 병렬 막대 차트의 측정값은 자동으로 하나의 측정값으로 병합되어 X축에 매핑됩니다. 여러 측정값이 있는 경우 측정값 이름은 나머지 차원과 병합되어 범례 항목으로 표시됩니다.

:::

**예시**
```ts
[{id: 'value1', alias: '측정값1'}, {id: 'value2', alias: '측정값2'}]
...
페이지네이션 구성으로, 차트의 페이지네이션 기능을 설정하는 데 사용됩니다
...
:::note{title="설명"}
차트의 배경색, 기본값은 투명 배경이며, 배경색은 'red', 'blue'와 같은 색상 문자열이거나 hex, rgb 또는 rgba '#ff0000', 'rgba(255,0,0,0.5)'일 수 있습니다.
...
VChart의 region[0].padding에 매핑되며, 주석, 레이블 등 플롯 영역 외부 확장 요소를 위한 공간을 확보합니다
...
:::note{title="설명"}
막대/세로 막대 차트 업데이트 효과, 성장 및 이동 애니메이션 지원
...
:::note{title="설명"}
y축, 카테고리 축, y축 구성은 차트의 y축을 정의하는 데 사용되며, y축의 위치, 형식, 스타일 등을 포함합니다.
...
:::note{title="설명"}
수평 툴팁 구성, 차트의 수평 툴팁을 정의하는 데 사용되며, 수평 툴팁의 색상, 레이블 스타일 등을 포함합니다.
...
:::note{title="설명"}
직사각형의 최대 높이, 픽셀 값 또는 백분율 문자열 가능
...
:::note{title="설명"}
동일한 분류 내 직사각형 간의 거리, 픽셀 값 또는 백분율 문자열 가능
...
:::note{title="설명"}
Y축 정렬 구성, 차원 또는 측정값 기준 정렬 및 사용자 지정 정렬 순서 지원
...
:::note{title="설명"}
직사각형 그래픽 요소 스타일, 막대 차트 스타일 구성, 차트의 막대 스타일을 정의하는 데 사용되며 막대의 색상, 테두리, 둥근 모서리 등을 포함합니다.
...
:::note{title="설명"}
값 기준선(평균선, 최대값선, 최소값선 등 포함), 세로 방향으로 표시, 기준선의 위치, 스타일 등을 설정할 수 있습니다. 평균선 등 값에 해당하는 기준선을 그리려면 이 구성을 사용하세요.
...
:::note{title="설명"}
차원 값 기준선, 가로 방향으로 표시, 기준선의 위치, 스타일 등을 설정할 수 있습니다.
...
:::note{title="설명"}
차이 기준선 구성, 두 데이터 앵커 포인트를 바인딩하고 절대 차이 또는 백분율 차이를 표시하는 데 사용됩니다.
...
:::note{title="설명"}
차이 주석선의 시작 앵커 포인트입니다.



차이 주석 앵커 구성으로, 시작점 또는 종료점에 바인딩된 데이터를 선택하는 데 사용됩니다.
...
:::note{title="설명"}
앵커 선택기이며, 최종적으로 하나의 논리적 앵커에 위치해야 합니다.
...
\- absolute: 절대 차이를 표시하며, 계산 방식은 end \- start입니다.

\- percent: 백분율 차이를 표시하며, 계산 방식은 (end \- start) / start입니다.
