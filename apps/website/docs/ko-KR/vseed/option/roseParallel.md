...
:::note{title="설명"}
그룹 장미 그래프, 다차원 데이터 비교 시나리오에 적합하며, 극좌표계의 부채꼴 호도와 반경을 통해 데이터 크기 표시
...
\- 첫 번째 차원은 각도 축에 배치되고, 나머지 차원은 지표 이름(여러 지표가 있는 경우)과 병합되어 범례 항목으로 표시

\- 모든 측정값은 자동으로 하나의 측정값으로 병합됩니다.

기본 활성화 기능:

\- 기본적으로 범례, 극좌표계, 데이터 레이블, 툴팁, 수치 확대/축소 활성화
...
그룹 장미 그래프, 극좌표계를 통해 다차원 데이터 비교 관계 표시
...
장미 그래프의 첫 번째 차원은 각도 축에 매핑되고, 나머지 차원은 지표 이름(여러 지표가 있는 경우)과 병합되어 범례 항목으로 표시됩니다.
...
장미 그래프의 지표는 자동으로 하나의 지표로 병합되어 반경 축에 매핑되며, 여러 지표가 있는 경우 지표 이름이 나머지 차원과 병합되어 범례 항목으로 표시됩니다.
...
:::note{title="설명"}
파이/도넛/로즈 차트 입장 효과, 방사형 및 확대/축소 애니메이션 지원
...
:::note{title="설명"}
파이/도넛/로즈 차트 업데이트 효과, 방사형 애니메이션 지원
