...
:::note{title="설명"}
면적 차트는 시간에 따른 데이터 추세 및 누적 관계를 표시하는 데 적합하며, 채워진 영역을 통해 데이터 비교를 강화합니다. X축은 범주형 축(분류 데이터), Y축은 수치형 축(연속 데이터)입니다.
...
\- 첫 번째 차원 필드는 X축에 매핑되고, 나머지 차원 필드는 측정값 이름(여러 측정값이 있는 경우)과 병합되어 범례 항목으로 표시됩니다.
...
면적 차트, 데이터 추세 및 누적 관계를 표시하는 차트 유형
...
TidyData 규격을 준수하고 이미 집계된 데이터셋으로, 차트의 데이터 소스와 구조를 정의합니다. 사용자가 입력한 데이터셋은 어떤 처리도 필요하지 않으며, VSeed는 강력한 데이터 재구성 기능을 통해 자체적으로 데이터를 재구성하며, 면적 차트의 데이터는 최종적으로 2개 차원, 1개 측정값으로 변환됩니다.
...
첫 번째 차원은 X축에 매핑되고, 나머지 차원은 측정값 이름(여러 측정값이 있는 경우)과 병합되어 범례 항목으로 표시됩니다.

:::

**예시**
```ts
[{ id: 'month', alias: '월' }, { id: 'year', alias: '연도' }]
...
면적 차트의 측정값은 자동으로 하나의 측정값으로 병합되어 Y축에 매핑되며, 측정값 이름은 나머지 차원과 병합되어 범례 항목으로 표시됩니다.
...
:::note{title="설명"}
꺾은선/영역형 차트 입장 효과, 로딩 및 성장 애니메이션 지원
...
:::note{title="설명"}
꺾은선/영역형 차트 업데이트 효과, 성장 애니메이션 지원
...
범주형 축, x축 설정, 차트의 x축을 정의하는 데 사용되며 x축의 위치, 형식, 스타일 등을 포함합니다.
...
선택한 두 데이터 포인트를 기반으로 차이 주석선을 그리고 차이 텍스트를 자동으로 계산합니다.
