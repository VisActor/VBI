...
:::info{title="인코딩 매핑"}
파이 차트는 다음 시각적 채널을 지원합니다:

`angle`  : 각도 채널, `여러 지표`를 지원하며, 지표 값에 따라 부채꼴 각도에 매핑됩니다
...
:::note{title="설명"}
파이 차트, 단일 차원 데이터의 비율 관계를 표시하는 데 적합하며, 섹터 면적 크기로 각 카테고리의 비율을 나타냅니다.
...
\- 카테고리 수가 적은 경우(6개 이하 권장)의 비율 분석
...
\- 모든 차원은 지표 이름(여러 지표가 있는 경우)과 병합되어 하나의 차원이 되며, 범례 항목으로 표시됩니다.

\- 모든 측정값은 자동으로 하나의 측정값으로 병합됩니다.

기본 활성화 기능:

\- 기본적으로 범례, 데이터 레이블, 툴팁, 비율 계산이 활성화됩니다.
...
TidyData 규격을 준수하고 이미 집계된 데이터셋으로, 차트의 데이터 소스와 구조를 정의하는 데 사용됩니다. 사용자가 입력한 데이터셋은 별도의 처리가 필요하지 않으며, VSeed는 강력한 데이터 리쉐이핑 기능을 갖추고 있어 자체적으로 데이터를 재구성합니다. 파이 차트의 데이터는 최종적으로 1개의 차원, 1개의 지표로 변환됩니다.
...
파이 차트의 모든 차원은 지표 이름(여러 지표가 있는 경우)과 병합되어 하나의 차원이 되며, 각도에 매핑되고 범례 항목으로 표시됩니다.
...
파이 차트의 모든 지표는 자동으로 하나의 지표로 병합되어 반경 축에 매핑되며, 여러 지표가 있는 경우 지표 이름이 나머지 차원과 병합되어 범례 항목으로 표시됩니다.
