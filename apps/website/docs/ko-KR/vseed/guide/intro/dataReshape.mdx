...
:::info
데이터와 차트 유형은 완전히 분리되어 있어, 차원이나 측정값이 얼마나 많든 VSeed는 항상 이를 리쉐이핑할 수 있습니다. 차트의 의미론이 다소 부적절할 수 있지만, 그만한 가치가 있습니다.
:::

아래 예제는 서로 다른 구조의 `dataset`과 다양한 `chartType` 간의 자유로운 조합을 보여줍니다. 아래 버튼을 클릭하여 데이터 세트와 차트 유형을 전환하고 VSeed가 자동으로 적용되는 방식을 관찰할 수 있습니다.
