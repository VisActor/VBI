...
운영 모니터링 대시보드를 참고하여 쿼리 기반 경영 요약, 카테고리 및 고객군별 매출 비중, 매출과 이익 추이, 지역 순위, 배송 분포, 고객 상세 정보, 실행 제안 등 10개의 카드를 구성합니다. 넓은 화면의 첫 행에는 카드 4개, 문서 영역에는 2열, 모바일에는 1열로 표시합니다.


> 태그: `dashboard` `multi-column` `responsive` `operations`
...
Dashboard에 밝은 테마를 저장합니다. 미리보기는 웹사이트 외관과 관계없이 저장된 테마를 사용합니다.
...
dashboardBuilder.theme.setTheme으로 어두운 테마를 적용해 카드, 차트, 편집기의 스타일을 통일합니다.
...
VSeed 토큰으로 브랜드 테마를 등록하여 색상, 텍스트, 배경, 카드를 통일합니다. 참조하는 차트 리소스는 변경하지 않습니다.
...
위젯 레이아웃을 검증하고 매출 차트와 경영 인사이트를 조합한 다음, 레이아웃을 조정하여 협업 복제본에 동기화합니다.


> 태그: `workflow` `dashboard` `collaboration`
...
상품 운영 대시보드에서 chart/insight 위젯을 추가, 수정, 삭제, 초기화한 뒤 최종 레이아웃을 다시 구성하여 병합을 검증합니다.


> 태그: `dashboard` `workflow` `layout` `mutation`
...
VBI resources 네임스페이스로 재사용 가능한 chart/insight 리소스를 등록, 조회, 나열, 삭제하고 스냅샷을 만든 뒤 분기 대시보드에서 참조합니다.


> 태그: `resources` `workflow` `dashboard`
