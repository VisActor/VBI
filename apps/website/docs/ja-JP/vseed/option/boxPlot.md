...
箱ひげ図は、次の視覚チャネルをサポートします:

`xAxis`  : x 軸チャネル。`複数のディメンション`をサポートし、ディメンション値に基づいて x 軸へマッピングします

`yAxis`  : y 軸チャネル。`複数のメジャー`をサポートし、メジャー値に基づいて y 軸へマッピングします
...
:::note{title="説明"}
箱ひげ図は、データ分布を表示するのに適しています。X 軸はカテゴリ軸（カテゴリデータ）、Y 軸は数値軸（連続データ）で、箱は縦方向に配置されます。

適用シーン:

\- データ項目名が短い場合

\- 異なるカテゴリの数値大小を直感のに比較する必要がある場合
...
\- 少なくとも 1 個数値フィールド（メジャー）

\- 最初のディメンションは X 軸に設定されます、 その他のディメンションはメジャー名（複数メジャーが存在する場合）と結合されます、 凡例項目として表示されます
...
:::note{title="説明"}
TidyData 仕様に準拠し、すでに集計済みのデータセットです。チャートのデータソースと構造を定義するために使用します。ユーザーが入力したデータセットに処理は不要です。VSeed は強力なデータ再形成機能を備えており、自動的にデータを再形成します。箱ひげ図のデータは最終的に 2 個のディメンション、1 個のメジャーへ変換されます。
...
:::note{title="説明"}
箱ひげ図の最初のディメンションは X 軸にマッピングされます。その他のディメンションはメジャー名（複数メジャーが存在する場合）と結合され、凡例項目として表示されます。
...
:::note{title="説明"}
箱ひげ図のすべてのメジャーは自動的に 1 つのメジャーに結合され、Y 軸にマッピングされます。複数メジャーが存在する場合、メジャー名はその他のディメンションと結合され、凡例項目として表示されます。
...
\- value: 離散値に対応するメジャーです。統計値を計算して箱ひげ図を表示するために使用します

\- q1: 統計値の 25 パーセンタイルに対応するメジャーマッピング

\- q3: 統計値の 75 パーセンタイルに対応するメジャーマッピング

\- min: ひげの最小値を表すメジャーマッピング

\- max: ひげの最大値を表すメジャーマッピング

\- meadian: 統計値の中央値に対応するメジャーマッピング

\- outliers: 外れ値のメジャーマッピング
...
:::note{title="説明"}
ページネーション設定です。ページネーションのフィールド名を指定するために使用し、ディメンションである必要があります。
...
:::note{title="説明"}
チャートの背景色です。背景色には色文字列を指定でき、デフォルトは透明背景です。例: 'red'、'blue'。hex、rgb、rgba（'#ff0000'、'rgba(255,0,0,0.5)'）も指定できます。
...
:::note{title="説明"}
x軸、 カテゴリ軸、x 軸設定です。チャートの x 軸を定義するために使用し、x 軸の位置、フォーマット、スタイルなどを含みます。
...
:::note{title="説明"}
y軸、 数値軸、y 軸設定です。チャートの y 軸を定義するために使用し、y 軸の位置、フォーマット、スタイルなどを含みます。
...
:::note{title="説明"}
X 軸ソート設定です。ディメンションまたはメジャーに基づくソート、およびカスタムソート順をサポートします
...
:::note{title="説明"}
垂直ツールチップ枠設定です。チャートの垂直ツールチップ枠を定義するために使用し、垂直ツールチップ枠の色、ラベルスタイルなどを含みます。
...
:::note{title="説明"}
箱ひげ図の箱部分のスタイル設定です。全体またはセレクター粒度で適用できます
...
:::note{title="説明"}
外れ値点のスタイル設定です。全体またはセレクター粒度で適用できます
...
:::note{title="説明"}
ヒストグラムのひげ長設定です。スカラー値と長さ 2 の配列をサポートします

値がスカラーの場合、whiskers * IQR を使用して上限値と下限値を計算します

値が 2 要素配列の場合、whiskers[0] は [0, 0.25) の範囲にある必要があり、下限値が対応するパーセンタイルを取ることを表します。

whiskers[1] は (0.75, 1] の範囲にある必要があり、上限値が対応するパーセンタイルを取ることを表します。
...
:::note{title="説明"}
数値注釈線（平均線、最大値線、最小値線などを含む）です。水平方向に表示し、注釈線の位置やスタイルなどを設定できます。平均線など数値に対応する注釈線を描画する場合はこの設定を使用してください。
...
:::note{title="説明"}
箱ひげ図の最大幅、できます設定絶対のピクセル値、也できます使用百分比（如 '10%'）
...
:::note{title="説明"}
グループ箱ひげ図中各個グループ内の間距、できます設定絶対のピクセル値、也できます使用百分比（如 '10%'）。
