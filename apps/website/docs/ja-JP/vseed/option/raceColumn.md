...
時間の経過に伴うデータのランキング変化を表示するのに適しており、棒は縦方向に並びます。

適用シーン:

\- データ項目名が長い場合

\- 異なるカテゴリの数値の大きさを直感的に比較し、時間に伴うランキング変化を表示したい場合

\- 時系列データの変化傾向を表示し、棒の順序を動的に更新する場合
...
\- X 軸はカテゴリ軸（カテゴリデータ）で、ディメンション値を表示します

\- Y 軸は数値軸（連続データ）で、メジャー値を表示します

\- プレーヤーで時間ディメンションを制御し、データ変化を動的に表示できます

\- アニメーション中、棒は数値の大きさに基づいて動的にソートされます
...
:::note{title="説明"}
動的縦棒グラフは、時間の経過に伴うデータのランキング変化を表示するのに適しています。
...
:::note{title="説明"}
TidyData 仕様に準拠し、すでに集計済みのデータセットです。
...
最初のディメンションは player に、2 番目のディメンションは X 軸にマッピングされます。
...
動的縦棒グラフのすべてのメジャーは自動的に 1 つのメジャーへ結合され、Y 軸にマッピングされます。複数メジャーが存在する場合、メジャー名はその他のディメンションと結合され、凡例項目として表示されます。
...
:::note{title="説明"}
プレーヤー設定です。時間ディメンションの指定に使用し、動的縦棒グラフの中核となる設定です。

プレーヤーで時間ディメンションの再生進捗を制御し、データの動的更新とソート変化を実現します。
...
:::note{title="説明"}
ソート設定です。動的縦棒グラフでは通常、数値に基づいた動的ソートが必要です。
...
:::note{title="説明"}
ページネーション設定です。データ量が多いシーンの処理に使用します。
...
:::note{title="説明"}
異なるディメンションまたはメジャーを区別するための色設定です。
...
:::note{title="説明"}
マウスホバー時に詳細情報を表示するためのツールチップ設定です。
...
:::note{title="説明"}
ブラシ選択インタラクションをサポートするためのブラシ選択設定です。
...
:::note{title="説明"}
X 軸設定です。カテゴリ軸としてディメンション値を表示し、棒は縦方向に並びます。
...
:::note{title="説明"}
クロスヘア設定です。データの正確な値を表示するために使用します。
...
:::note{title="説明"}
縦棒スタイル設定です。単一スタイルまたは配列形式を指定できます。
...
:::note{title="説明"}
特定のデータポイントにマークを追加するための注釈ポイント設定です。
...
:::note{title="説明"}
数値注釈線です。特定の X 軸数値をマークする縦方向の注釈線です。
...
:::note{title="説明"}
ディメンション値注釈線です。特定の Y 軸カテゴリをマークする横方向の注釈線です。
...
:::note{title="説明"}
複数チャート間のディメンション連動インタラクションをサポートするディメンション連動設定です。
