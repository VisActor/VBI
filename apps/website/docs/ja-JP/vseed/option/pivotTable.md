...
ピボットテーブルは、次の視覚チャネルをサポートします:

`row`    : 行ディメンション。`複数のディメンション`をサポートし、ディメンション値に基づいて行方向でグループ化します

`column` : 列ディメンション。`複数のディメンション`をサポートし、ディメンション値に基づいて列方向でグループ化します
...
:::note{title="説明"}
ピボットテーブルは、多次元データのクロス分析シーンに適しています。行・列ディメンションとメジャーの計算方法を柔軟に設定できます。
...
\- 少なくとも 1 個の行ディメンション、または 1 個の列ディメンション、または 1 個のメジャー
...
\- 行列ソート、データフィルター、集計計算、小計/総計がデフォルトで有効です
...
:::note{title="説明"}
TidyData 仕様に準拠し、すでに集計済みのデータセットです。チャートのデータソースと構造を定義するために使用します。ユーザーが入力したデータセットに処理は不要です。VSeed は強力なデータ再形成機能を備えており、自動的にデータを再形成します。ピボットテーブルのデータは最終的に対応するツリー構造へ変換されるため、ユーザーが手動でデータ処理を行う必要はありません。

:::

**例**
```ts
[{region:'華東', product:'A', sales:1000}, {region:'華東', product:'B', sales:1500}]
...
:::note{title="説明"}
ピボットテーブルの行ディメンションと列ディメンションは、データを自動的にツリー構造へ処理し、行軸と列軸にマッピングします。

:::

**例**
```ts
[{id: 'region', alias: '地域', isRow: true}, {id: 'product', alias: '製品', isColumn: true}]
...
**例**
```ts
[{id: 'sales', alias: '売上高', aggregation: 'sum'}]
...
:::note{title="説明"}
マウスが行ヘッダーまたは列ヘッダーのセル上にあるときの背景色です。マウスが置かれている行と列が交差するセルを強調表示するために使用します。
...
:::note{title="説明"}
マウスが行ヘッダーまたは列ヘッダーのセル上にあるとき、マウスが置かれている行と列のすべてのセルを強調表示するために使用します。
...
:::note{title="説明"}
メジャーを列として表示するかどうかです。true の場合はメジャーが列方向に展開され、false の場合は行方向に展開されます。
...
:::note{title="説明"}
小計のディメンションです。どのディメンションで小計をグループ化するかを指定します。
