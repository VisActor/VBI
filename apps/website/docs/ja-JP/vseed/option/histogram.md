...
ヒストグラムは次の視覚チャネルをサポートします:

`xAxis`  : x 軸チャネル。`1 つのディメンション`をサポートし、ディメンション値をビニングして x 軸に表示します

:::

:::note{title="説明"}
ヒストグラムは、データ分布を表示するシーンに適しています。X 軸は数値軸（連続データ）、Y 軸も数値軸（連続データ）で、棒を縦方向に並べます。
...
:::note{title="説明"}
TidyData 仕様に準拠し、すでに集計済みのデータセットです。チャートのデータソースと構造を定義するために使用します。ユーザーが入力したデータセットに処理は不要です。VSeed は強力なデータ再形成機能を備えており、自動的にデータを再形成します。ヒストグラムのデータは最終的に 2 個のディメンション、1 個のメジャーへ変換されます。
...
:::note{title="説明"}
ヒストグラムは 1 つのディメンションのみをサポートし、そのデータは離散データである必要があります。
...
:::note{title="説明"}
チャートの背景色です。背景色には色文字列を指定できます。デフォルトは透明背景です。例: 'red', 'blue'。hex、rgb、rgba（'#ff0000', 'rgba(255,0,0,0.5)'）も指定できます
...
:::note{title="説明"}
x 軸。数値軸です。チャートの x 軸の位置、フォーマット、スタイルなどを定義する x 軸設定です。
...
:::note{title="説明"}
ヒストグラムのビン数です。ヒストグラムのビン矩形（棒）の数を定義します
...
:::note{title="説明"}
ビンのステップ幅です。ビン幅の計算に使用し、最終的なヒストグラム内の矩形（棒）の幅にも影響します。binCount と binStep を同時に設定した場合は、binStep が優先されます
...
:::note{title="説明"}
ヒストグラムのビン値タイプです。ヒストグラムのビン矩形（棒）の値タイプを定義します。デフォルトは 'count' です
...
:::note{title="説明"}
矩形マークスタイル。縦棒グラフのスタイル設定で、縦棒グラフの色、枠線、角丸などを定義します。
...
:::note{title="説明"}
数値注釈線（ビン値）です。垂直方向に表示し、注釈線の位置やスタイルなどを設定できます。ビン値に対応する注釈線が必要な場合は、この設定を使用できます
...
:::note{title="説明"}
数値注釈線（平均線、最大値線、最小値線など）です。水平方向に表示し、注釈線の位置やスタイルなどを設定できます。ビン値に対応する注釈線を描画する場合は、この設定を使用してください。ビン値は `binValueType` の影響を受けます
...
:::note{title="説明"}
カーネル密度回帰線設定です。データの傾向と分布状況を表示するために使用します
...
:::note{title="説明"}
経験累積分布関数の回帰線設定です。データの累積分布状況を表示するために使用します
