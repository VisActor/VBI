...
:::note{title="説明"}
サークルパッキング図は階層データの表示に使用します。円の大きさで数値の大きさを表します。
...
\- 円の大きさにマッピングするため、少なくとも 1 個の数値フィールド（メジャー）が必要です
