...
:::note{title="説明"}
面グラフは、データが時間とともに変化する傾向と累積関係を表示するのに適しており、塗りつぶし領域によってデータ比較を強調します。X 軸はカテゴリ軸（カテゴリデータ）、Y 軸は数値軸（連続データ）です。

適用シーン:

\- 単一データ系列の傾向変化を表示する

\- 総量が時間とともに累積する効果を強調する
...
\- 少なくとも 1 個メジャーフィールド（メジャー）

\- 最初のディメンションフィールドX 軸にマッピングされます、その他のディメンションフィールドはメジャー名称(複数メジャーが存在する場合)結合、 凡例項目として表示されます.
...
\- デフォルトで有効な凡例、座標軸、領域塗りつぶし、データラベル、ツールチップ
...
面グラフは、データ傾向と累積関係を表示するチャートタイプです。
...
TidyData 仕様に準拠し、すでに集計済みのデータセットです。チャートのデータソースと構造を定義するために使用します。ユーザーが入力したデータセットに処理は不要です。VSeed は強力なデータ再形成機能を備えており、自動的にデータを再形成します。面グラフのデータは最終的に 2 個のディメンション、1 個のメジャーへ変換されます。
...
最初のディメンションは X 軸にマッピングされます。その他のディメンションはメジャー名（複数メジャーが存在する場合）と結合され、凡例項目として表示されます。

:::

**例**
```ts
[{ id: 'month', alias: '月' }, { id: 'year', alias: '年' }]
...
面グラフのメジャーは自動的に 1 つのメジャーに結合され、Y 軸にマッピングされます。メジャー名はその他のディメンションと結合され、凡例項目として表示されます。
...
背景色には 'red'、'blue' などの色文字列を指定できます。また、hex、rgb、rgba（'#ff0000'、'rgba(255,0,0,0.5)'）も指定できます。
...
:::note{title="説明"}
折れ線/面グラフ入場効果、サポートロード和成長アニメーション
...
カテゴリ軸、x 軸設定です。チャートの x 軸を定義するために使用し、x 軸の位置、フォーマット、スタイルなどを含みます。
...
数値軸、y 軸設定です。チャートの y 軸を定義するために使用し、y 軸の位置、フォーマット、スタイルなどを含みます。
...
マウスがチャート上を移動したときに表示される垂直ツールチップ線です



クロスヘア線設定です。チャート内でクロスヘア線（ツールチップ線）を表示するための設定タイプです
...
:::note{title="説明"}
チャートのテーマ、 テーマ是優先度較低い機能設定、 包含すべてのチャートタイプ共通の汎用設定、 と単一チャートタイプ共通のチャート設定



light と dark の 2 種類の組み込みテーマがあります。ユーザーは Builder でテーマをカスタムできます
...
:::note{title="説明"}
点マークのスタイル設定です。チャートの点マークスタイルを定義するために使用し、点マークの色、枠線などを含みます。
...
:::note{title="説明"}
線マークのスタイル設定です。チャートの線マークスタイルを定義するために使用し、線マークの色、不透明度、曲線などを含みます。
...
:::note{title="説明"}
面マークのスタイル設定です。チャートの面マークスタイルを定義するために使用し、面マークの色、不透明度、枠線などを含みます。
