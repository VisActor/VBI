...
:::note{title="説明"}
百分比面グラフは、複数カテゴリの構成比が時間とともに変化する傾向を表示するのに適しており、Y 軸は構成比関係を百分比形式で表示します。
...
\- デフォルトで有効な凡例、座標軸、百分比ラベル、ツールチップ、構成比計算
...
百分比面グラフは、複数カテゴリの構成比が特定ディメンションに沿って変化する様子を百分比形式で表示します。
...
TidyData 仕様に準拠し、すでに集計済みのデータセットです。チャートのデータソースと構造を定義するために使用します。ユーザーが入力したデータセットに処理は不要です。VSeed は強力なデータ再形成機能を備えており、自動的にデータを再形成します。百分比面グラフのデータは最終的に 2 個のディメンション、1 個のメジャーへ変換されます。
...
百分比面グラフのメジャーは自動的に 1 つのメジャーに結合され、Y 軸にマッピングされます。メジャー名はその他のディメンションと結合され、凡例項目として表示されます。
