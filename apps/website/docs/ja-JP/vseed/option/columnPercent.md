...
:::note{title="説明"}
百分率縦棒グラフは、各カテゴリの構成比関係を表示するシーンに適しています。Y 軸はデータの構成比を百分率形式で表示します。

適用シーン:

\- 異なるカテゴリのデータ構成比を比較する場合

\- 多次元データの構成を分析する場合

\- 時系列での構成比の変化傾向を表示する場合
...
\- 凡例、座標軸、百分率ラベル、ツールチップ、構成比計算がデフォルトで有効です
...
百分率縦棒グラフは、各カテゴリのデータ構成比を百分率形式で表示します。
...
TidyData 仕様に準拠し、すでに集計済みのデータセットです。チャートのデータソースと構造を定義するために使用します。ユーザーが入力したデータセットに処理は不要です。VSeed は強力なデータ再形成機能を備えており、自動的にデータを再形成します。百分率縦棒グラフのデータは最終的に 2 個のディメンション、1 個のメジャーへ変換されます。
...
最初のディメンションは X 軸にマッピングされます。残りのディメンションは、メジャー名（複数メジャーが存在する場合）と結合され、凡例項目として表示されます。
...
百分率縦棒グラフのメジャーは自動的に 1 つのメジャーへ結合され、Y 軸にマッピングされます。複数メジャーが存在する場合、メジャー名はその他のディメンションと結合され、凡例項目として表示されます。
...
:::note{title="説明"}
チャートの背景色です。デフォルトは透明背景です。背景色には色文字列を指定できます。例: 'red', 'blue'。hex、rgb、rgba（'#ff0000', 'rgba(255,0,0,0.5)'）も指定できます
...
:::note{title="説明"}
矩形マークスタイル。チャートの矩形マークの色、枠線、角丸などを定義します。
