...
:::info{title="推奨"}
\- 推奨フィールド設定: `2`個のメジャー、`2`個のディメンション
...
双軸グラフは次の視覚チャネルをサポートします:

`xAxis`          : x 軸チャネル。`複数のディメンション`をサポートし、ディメンション値を x 軸にマッピングします

`primaryYAxis`   : 主 y 軸チャネル。`複数のメジャー`をサポートし、メジャーを主軸にマッピングします

`secondaryYAxis` : 副 y 軸チャネル。`複数のメジャー`をサポートし、メジャーを副軸にマッピングします
...
:::note{title="説明"}
双軸グラフは、量級や単位が異なる 2 つのメジャーの比較関係を表示するシーンに適しており、主座標軸と副座標軸を含みます。
...
\- 数値や成長率などの複合メジャーを同時に表示したい場合

\- 異なるチャートタイプの組み合わせ（折れ線グラフ + 縦棒グラフ / 折れ線グラフ + 面グラフ / 面グラフ + 縦棒グラフなど）をサポートします
...
\- メジャーグループをサポートします。最初のメジャーグループは（主軸の）左軸に、2 番目のメジャーグループは（副軸の）右軸に配置されます

\- 最初のディメンションは X 軸に配置されます。残りのディメンションは、メジャー名（複数メジャーが存在する場合）と結合され、凡例項目として表示されます。

\- 2 つのメジャーフィールドグループは、左右 2 つの Y 軸にそれぞれマッピングできます。1 つのメジャーグループ内のすべてのメジャーは自動的に 1 つのメジャーに結合されます

デフォルトで有効な機能:

\- 座標軸、凡例、データラベル、ツールチップがデフォルトで有効です
...
:::note{title="説明"}
双軸グラフは、異なる量級の 2 つのメジャーの比較関係を表示する複合グラフです。
...
:::note{title="説明"}
データセット。TidyData 仕様に準拠し、すでに集計済みのデータセットです。チャートのデータソースと構造を定義するために使用します。ユーザーが入力したデータセットに処理は不要です。VSeed は強力なデータ再形成機能を備えており、自動的にデータを再形成します。双軸グラフのデータは最終的に 2 個のディメンション、1 個または 2 個のメジャー（ユーザーがメジャーグループを設定しているかによって決まります）へ変換されます。
...
:::note{title="説明"}
ディメンション。最初のディメンションは X 軸に配置されます。残りのディメンションは、メジャー名（複数メジャーが存在する場合）と結合され、凡例項目として表示されます。
...
encoding で primaryYAxis と secondaryYAxis にマッピングされたメジャーについて、

`parentId` プロパティを設定することでメジャーをグループ化でき、異なるグループのメジャーは異なるサブチャートに表示されます。

また、`chartType` プロパティを設定して、メジャーグループごとに異なるチャートタイプを指定できます。
...
\- primaryYAxis: メジャーを主 y 軸にマッピングします

\- secondaryYAxis: メジャーを副 y 軸にマッピングします
...
:::note{title="説明"}
このメジャーの双軸グラフ内でのチャートタイプを設定します。双軸グラフにのみ適用されます。

\- line: 折れ線グラフ

\- column: 縦棒グラフ

\- columnParallel: 並列縦棒グラフ

\- columnPercent: 百分率縦棒グラフ
...
:::note{title="説明"}
双軸グラフの 2 本の軸の目盛りを揃えるかどうかを定義します。measures に複数グループがある場合、alignTicks は配列として設定でき、各項目が 1 つの双軸グラフの目盛り揃えに対応します。
...
:::note{title="説明"}
双軸グラフの主 Y 軸設定です。主 Y 軸の位置、スタイルなどを定義します。measures に複数グループがある場合、primaryYAxis は配列として設定でき、各項目が 1 つの双軸グラフの主 Y 軸に対応します。
...
:::note{title="説明"}
双軸グラフの副 Y 軸設定です。副 Y 軸の位置、スタイルなどを定義します。measures に複数グループがある場合、secondaryYAxis は配列として設定でき、各項目が 1 つの双軸グラフの副 Y 軸に対応します。
...
横棒グラフのスタイル設定です。横棒グラフの色、枠線、角丸などを定義します。
...
線マークスタイル設定, チャートの線マークスタイルを定義するために使用します, 線マークの色、透明度、曲線などを含みます。
...
エリアマークスタイル設定, チャートのエリアマークスタイルを定義するために使用します, エリアマークの色、透明度、枠線などを含みます。
...
:::note{title="説明"}
国際化設定です。チャート言語設定として 'zh\-CN' と 'en\-US' の 2 言語をサポートします。また intl.setLocale('zh\-CN') メソッドを呼び出して言語を設定できます
