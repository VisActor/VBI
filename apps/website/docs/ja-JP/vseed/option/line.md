...
折れ線グラフは、次の視覚チャネルをサポートします:

`x`      : x 軸チャネル。`複数のディメンション`をサポートし、ディメンション値に基づいて x 軸にマッピングします

`y`      : y 軸チャネル。`複数のメジャー`をサポートし、メジャー値に基づいて y 軸にマッピングします
...
:::note{title="説明"}
折れ線グラフは、時間や順序付きカテゴリに沿ったデータの変化傾向を表示するのに適しており、データポイントを線分で結んでトレンドラインを形成します。
...
\- 最初のディメンションは X 軸に配置されます。その他のディメンションは、複数メジャーが存在する場合はメジャー名と結合され、凡例項目として表示されます

\- すべてのメジャーは自動的に 1 つのメジャーへ結合されます

デフォルトで有効な機能:

\- 凡例、座標軸、データポイントマーカー、ツールチップ、トレンドラインがデフォルトで有効です
...
:::note{title="説明"}
折れ線グラフは、時間や順序付きカテゴリに沿ったデータの変化傾向を表示するのに適しています。
...
:::note{title="説明"}
データソースです。TidyData 仕様に準拠し、すでに集計済みのデータセットで、チャートのデータソースと構造を定義するために使用します。ユーザーが入力したデータセットに処理は不要です。VSeed は強力なデータ再形成機能を備えており、自動的にデータを再形成します。折れ線グラフのデータは最終的に 2 個のディメンション、1 個のメジャーへ変換されます。
...
:::note{title="説明"}
ディメンションです。折れ線グラフの最初のディメンションは X 軸にマッピングされ、その他のディメンションは、複数メジャーが存在する場合はメジャー名と結合され、凡例項目として表示されます。
...
:::note{title="説明"}
メジャーです。折れ線グラフのすべてのメジャーは自動的に 1 つのメジャーへ結合され、Y 軸にマッピングされます。複数メジャーが存在する場合、メジャー名はその他のディメンションと結合され、凡例項目として表示されます。
...
チャートの背景色です。デフォルトは透明背景です。背景色には 'red'、'blue' などの色文字列を指定できます。また、'#ff0000'、'rgba(255,0,0,0.5)' のような hex、rgb、rgba も指定できます。
...
VChart の region[0].padding にマッピングされ、注釈やラベルなど、描画領域の外側へ広がる要素のためのスペースを確保します。
...
:::note{title="説明"}
折れ線/エリアチャートのアニメーションを有効にするかどうかです。
...
:::note{title="説明"}
折れ線/エリアチャートの入場効果です。読み込みアニメーションと成長アニメーションをサポートします。
...
:::note{title="説明"}
折れ線/エリアチャートの更新効果です。成長アニメーションをサポートします。
...
:::note{title="説明"}
アンビエントアニメーション効果です。リップル、表示/非表示、ブレスをサポートします。
...
カテゴリ軸です。チャートの x 軸を定義する x 軸設定で、x 軸の位置、フォーマット、スタイルなどを含みます。
...
数値軸です。チャートの y 軸を定義する y 軸設定で、y 軸の位置、フォーマット、スタイルなどを含みます。
...
マウスをチャート上に移動したときに表示される垂直ガイドラインです。



クロスヘアライン設定です。チャート内にクロスヘアライン（ガイドライン）を表示するための設定タイプです。
...
:::note{title="説明"}
X 軸ソート設定です。ディメンションまたはメジャーに基づくソートと、カスタムソート順をサポートします。
...
:::note{title="説明"}
凡例ソート設定です。ディメンションまたはメジャーに基づくソートと、カスタムソート順をサポートします。
...
チャートの点図形要素スタイルを定義する設定です。点図形要素の色、枠線などを含みます。
...
チャートの線図形要素スタイルを定義する設定です。線図形要素の色、透明度、曲線などを含みます。
...
選択したデータに基づいてチャートの注釈ポイントを定義する設定です。注釈ポイントの位置、フォーマット、スタイルなどを含みます。
...
:::note{title="説明"}
ディメンション値注釈線です。垂直方向に表示され、注釈線の位置やスタイルなどを設定できます。
...
:::note{title="説明"}
数値注釈線（平均線、最大値線、最小値線など）です。水平方向に表示され、注釈線の位置やスタイルなどを設定できます。平均線など数値に対応する注釈線を描画する場合はこの設定を使用してください。
...
選択したデータに基づいてチャートの注釈領域を定義する設定です。注釈領域の位置、スタイルなどを含みます。
...
選択した 2 つのデータポイントに基づいて差分注釈線を描画し、差分テキストを自動計算します。
...
差分注釈アンカー設定です。開始点または終了点にバインドするデータを選択するために使用します。
...
\- percent: パーセント差分を表示します。計算方法は (end \- start) / start です
...
:::note{title="説明"}
チャートでピボット機能またはメジャー組み合わせが有効な場合に、ディメンション連動機能を有効にするかどうかです。

特定のディメンション値に hover したとき、他のチャート内の同じディメンション値のデータを連動して強調表示します。
