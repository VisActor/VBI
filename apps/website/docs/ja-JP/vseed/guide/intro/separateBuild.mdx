...
:::tip
通常、チャートのデータ処理には比較的時間がかかります。VSeed はデータ処理とデータレンダリングの分割をサポートし、`build` は `buildAdvanced` と `buildSpec` を提供して分離構築を行います。
:::
...
次のサンプルでは、React コンポーネント内で分離構築フローを使用する方法を示します。
