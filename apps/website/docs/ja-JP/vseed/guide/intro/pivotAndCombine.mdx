...
`measures` 内の `parentId` 属性により、メジャーをグループ化して表示できます。
...
ディメンションの `encoding` 属性を設定することで、ピボットテーブル内でのディメンションの行列位置を制御できます。デフォルトではディメンションは行に置かれます。`encoding: 'column'` を設定すると列に配置できます。
