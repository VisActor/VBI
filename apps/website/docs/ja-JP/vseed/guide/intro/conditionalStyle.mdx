...
`barStyle` 属性で矩形図形要素のスタイルを設定します。配列の最初の項目はデフォルトスタイルで、後続の項目は条件付きスタイルです。`selector` 条件を満たすデータには対応するスタイルが適用されます。
...
`pointStyle` 属性で点図形要素のスタイルを設定します。同様に条件付きスタイル設定をサポートします。
