...
:::info
データとチャートタイプは完全に分離されています。ディメンションやメジャーがいくつあっても、VSeed は常にそれらをリシェイプできます。代償としてチャートの意味論が弱くなる可能性はありますが、それでも価値があります。
:::

次のサンプルでは、異なる構造の `dataset` と異なる `chartType` を自由に組み合わせる方法を示します。下のボタンをクリックしてデータセットとチャートタイプを切り替え、VSeed がどのように自動適配するかを確認できます。
