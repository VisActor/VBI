...
:::tip
`vseed` はロスレスなシリアライズをサポートするため、`クロスプラットフォーム転送` や `コピー&ペースト` が可能です。Node 環境とブラウザー環境のどちらでも、`builder` から完全な spec を直接取得できます。
:::

次のサンプルでは、VSeed 設定を JSON 文字列へシリアライズする方法（コピーのシミュレーション）と、JSON 文字列をデシリアライズしてレンダリングする方法（ペーストのシミュレーション）を示します。
