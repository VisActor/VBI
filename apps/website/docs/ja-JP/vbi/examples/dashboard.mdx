...
運営監視ダッシュボードを参考に、クエリに基づく経営概要、カテゴリ・顧客層別の売上構成、売上と利益の推移、省別ランキング、配送分布、顧客明細、アクション提案の計 10 枚のカードを配置します。ワイド画面では先頭行に 4 枚、ドキュメント内では 2 列、モバイルでは 1 列で表示します。


> タグ: `dashboard` `multi-column` `responsive` `operations`
...
Dashboard にライトテーマを保存します。プレビューはサイトの外観に関係なく、保存されたテーマを使用します。
...
dashboardBuilder.theme.setTheme でダークテーマに切り替え、カード、チャート、エディターの表示を統一します。
...
VSeed token でブランドテーマを登録し、配色、文字、背景、カードを統一します。参照先のチャートリソースは変更しません。
...
ウィジェットのレイアウトを検証し、売上グラフと経営インサイトを組み合わせ、レイアウト調整後に共同編集用の複製へ同期します。


> タグ: `workflow` `dashboard` `collaboration`
...
商品運営ダッシュボードで chart/insight ウィジェットを追加・更新・削除・全消去し、最終レイアウトを再構築してマージを検証します。


> タグ: `dashboard` `workflow` `layout` `mutation`
...
VBI resources 名前空間で再利用可能な chart/insight リソースを登録・取得・一覧表示・削除・スナップショット化し、四半期ダッシュボードから参照します。


> タグ: `resources` `workflow` `dashboard`
