...
> Tags: `dashboard` `multi-column` `responsive` `dark` `operations`
...
export default () => {
  ...
  return <div ref={previewRef} style={{ overflow: 'auto', background: dashboard.build().meta.theme === 'dark' ? '#000' : '#fff', color: dashboard.build().meta.theme === 'dark' ? '#eee' : '#222' }}>
...
export default () => {
  ...
  return <DashboardRenderer builder={dashboard} locale={locale} />
}
```
    ...
    const run = async () => {
      try {
        ...
        const applyBuilder = (builder: VBIDashboardBuilder) => {
          ...
          if (builder.chart.find(resources.charts.categorySalesChart.getUUID())?.getId() !== categoryWidget.id) throw new Error('chart reference should resolve')
          if (builder.insight.find(resources.insights.promotionInsight.getUUID())?.getId() !== promotionWidget.id) throw new Error('insight reference should resolve')
