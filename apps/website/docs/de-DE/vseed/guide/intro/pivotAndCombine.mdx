...
## Indikatorgruppierung

Bestanden `measures` in `parentId` Attribute, mit denen Metriken in Gruppen angezeigt werden können.
...
## Zeilen- und Spaltenperspektive

Durch Konfigurieren der Dimension `encoding` Eine Eigenschaft, mit der Sie die Position der Zeilen und Spalten der Dimension in der PivotTable steuern können.Standardmaß on line, Einstellung `encoding: 'column'` kann auf einer Säule platziert werden.
