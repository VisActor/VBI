...
:::info
Daten- und Diagrammtypen sind vollständig entkoppelt, Egal wie viele Dimensionen oder Metriken, VSeed kann sie jederzeit umformen. Die Kosten könnten die schlechte Semantik der Diagramme sein, Aber es lohnt sich.
:::

Das folgende Beispiel zeigt die unterschiedlichen baulichen `dataset` und andere `chartType` Kostenlose Kombination zwischen.Sie können auf die Schaltfläche unten klicken, um zwischen Dataset- und Diagrammtypen umzuschalten, um zu sehen, wie VSeed automatisch passt.
