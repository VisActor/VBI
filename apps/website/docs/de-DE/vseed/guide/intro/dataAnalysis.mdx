...
## Achsen- und Legendensortierung

VSeed Bietet eine bequeme Sortierkonfiguration.

- `sort`: Wird verwendet, um die Sortierung von Achsendaten zu konfigurieren, unterstützt die Angabe von Sortierfeldern `orderBy` und Sortierreihenfolge `order`。
- `sortLegend`: Wird verwendet, um die Sortierreihenfolge der Legende zu konfigurieren.
