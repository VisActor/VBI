...
Trichterdiagramme unterstützen folgende visuelle Kanäle:

`size`: Größenkanal, Support`mehrere Kennzahlen`, Zuordnen zur Trichterbreite nach metrischem Wert
...
Trichterdiagramm zur Darstellung des Datenanteils in einer einzigen Dimension

Szenario:

Anwendbare Trichter-Szenarien:

\- Geeignet für die Analyse von Prozessen mit mehreren aufeinanderfolgenden, normalisierten Schritten und klarer Darstellung des Datenverlusts oder der Konvertierung an jedem Link
...
\- Alle Dimensionen werden zu einer Dimension mit dem Metriknamen zusammengeführt (wenn es mehrere Metriken gibt), Als Legendenelement anzeigen

\- Alle Metriken werden automatisch zu einer Metrik zusammengeführt

Standardmäßig eingeschaltete Funktionen:

\- Standard-Öffnungslegende, Datenbeschriftung, zeitnahe Informationen, Proportionsbere
...
Trichterdiagramm, das den Datenanteil in einer einzigen Dimension zeigt
...
Alle Dimensionen des Trichters werden zu einer Dimension mit dem Metriknamen zusammengeführt (bei mehreren Metriken),und als Legendenelement angezeigt
...
Alle Metriken des Trichters werden automatisch zu einer Metrik zusammengeführt, Wenn es mehrere Metriken gibt, Der Metrikname wird mit dem Rest der Dimensionen zusammengeführt, Wird als Legendenelement angezeigt.
...
:::note{title="Beschreibung"}
Farbschema mit linearem Verlauf, Farbschema zur Definition von Diagrammen
...
:::note{title="Beschreibung"}
Konfiguration der Farblegende, Legende zur Definition eines Diagramms, Geben Sie den Ort der Legende an, Format, Stil, etc.
