...
Box Plots unterstützen die folgenden visuellen Kanäle:
...
Box-Diagramm, geeignet zur Anzeige der Verteilung von Daten, X-Achse ist die Kategorieachse (klassifizierte Daten), Y-Achse ist die numerische Achse (kontinuierliche Daten), die Box ist längs angeordnet

Szenario:

\- Wenn der Datenelementname kurz ist

\- Es ist notwendig, die Wertgrößen verschiedener Kategorien visuell zu vergleichen

\- Zeitreihendaten-Trends anzeigen
...
\- Mindestens 1 numerisches Feld (metrisch)

\- Die erste Dimension geht auf die X-Achse, Die verbleibenden Dimensionen werden mit dem Metriknamen zusammengeführt (wenn mehrere Metriken vorhanden sind), Als Legendenelement anzeigen

\- Alle Metriken werden automatisch zu einer Metrik zusammengeführt

Standardmäßig eingeschaltete Funktionen:

\- Legende, Achsen, Datenbeschriftungen, Eingabeaufforderungen standardmäßig öffnen
...
:::note{title="Beschreibung"}
Box-Diagramm, geeignet zur Anzeige der Datenverteilung, X-Achse ist die Kategorieachse (Klassifizierungsdaten), Y-Achse ist die numerische Achse (kontinuierliche Daten), die Box ist längs angeordnet
...
:::note{title="Beschreibung"}
Die erste Dimension des Box-Plots wird auf die X-Achse abgebildet, Die verbleibenden Dimensionen werden mit dem Metriknamen zusammengeführt (wenn mehrere Metriken vorhanden sind), Als Legendenelement anzeigen
...
:::note{title="Beschreibung"}
Alle Indikatoren im Box-Diagramm werden automatisch zu einem Indikator zusammengeführt, Zuordnen zur Y-Achse, Wenn es mehrere Metriken gibt, Der Metrikname wird mit dem Rest der Dimensionen zusammengeführt, Wird als Legendenelement angezeigt.
...
\- value: Kennzahl für diskrete Werte, die zur Berechnung statistischer Werte für die Anzeige eines Boxplots verwendet wird

\- q1: Indikator-Mapping für Statistik 25. Quartil

\- q3: Indikator-Mapping entsprechend der Statistik 75. Quartil

\- min: Indikator-Zuordnung des vom Kasten geforderten Mindestwerts

\- max: Indikator-Mapping des Maximalwerts der Box

\- meadian: Metrik-Mapping für Median-Statistik

\- outliers: Indikator-Mapping von Ausreißern
...
:::note{title="Beschreibung"}
Diagramm-Hintergrundfarbe, Hintergrundfarbe kann eine Farbstring sein, Standardmäßig transparenter Hintergrund, Beispiel'red', 'blue', Es kann auch Sechskant sein, rgboder rgba'#ff0000', 'rgba(255,0,0,0.5)'
...
:::note{title="Beschreibung"}
yAchse, Wertachse, yAchsenkonfiguration, zur Definition der y-Achse des Diagramms, einschließlich Position, Format, Stil usw. der y-Achse
...
:::note{title="Beschreibung"}
Box-Diagramm-Box-Stil-Konfiguration, unterstützt globale oder Selektor-Granularität, um wirksam zu werden
...
:::note{title="Beschreibung"}
boxPlotOb das Primitive sichtbar ist
...
:::note{title="Beschreibung"}
boxPlotGrafische Farbtransparenz
...
:::note{title="Beschreibung"}
Abgerundete Eckengröße der Box
...
:::note{title="Beschreibung"}
Farbe des Kasten-Whisker-Gewindes
...
:::note{title="Beschreibung"}
Konfiguration des Ausnahmepunkt-Stils, Unterstützung der globalen oder Selektor-Granularität, um wirksam zu werden
...
:::note{title="Beschreibung"}
Eine Must-Length-Konfiguration für Histogramme mit Unterstützung für skalare Werte und Arrays der Länge 2

Verwenden Sie Schnurrhaare, wenn der Wert skalar ist * IQR zur Berechnung der Ober- und Untergrenzen

Wenn der Wert ein 2-jähriges Array ist, werden Schnurrhaare[0] Muss in[0, 0.25), was anzeigt, dass der untere Grenzwert den entsprechenden Prozentsatz annimmt;

whiskers[1] Muss bei (0,75, 1], was anzeigt, dass der obere Grenzwert den entsprechenden Prozentsatz annimmt;
...
:::note{title="Beschreibung"}
Die maximale Breite des Box-Plots, entweder in absoluten Pixeln oder als Prozentsatz (z. B. '10%'）
...
:::note{title="Beschreibung"}
Der Abstand innerhalb jeder Gruppierung im Gruppierungsfelddiagramm, können Sie den absoluten Pixelwert einstellen, oder Sie können einen Prozentsatz verwenden (wie '10%'）。
