...
Es ist geeignet, die Verteilung der Daten über die Zeit anzuzeigen und die Werte zweier Indikatoren durch die Position der Datenpunkte darzustellen.

Anwendbare Gelegenheiten

\- Analysieren Sie die Verteilung von Daten im zweidimensionalen Raum und zeigen Sie, wie sie sich im Laufe der Zeit dynamisch ändert

\- Zeigen Sie die Korrelation zwischen mehreren Variablen über die Zeit

\- Beobachten Sie die Trajektorie der Datenpunkte im 2D-Raum
...
\- XSowohl die Achse als auch die Y-Achse sind numerische Achsen (kontinuierliche Daten), die mehrere Indikatorzuordnungen unterstützen

\- Unterstützt die Steuerung der Zeitdimension über den Player, um Datenänderungen dynamisch anzuzeigen

\- Visualisieren Sie die Datendynamik, indem Sie die Position von Datenpunkten ändern
...
:::note{title="Beschreibung"}
Dynamisches Streudiagramm zur Darstellung der Datenverteilung über die Zeit
...
:::note{title="Beschreibung"}
Datenquelle, TidyData-konformer Datensatz
...
:::note{title="Beschreibung"}
Dimension zur Unterscheidung verschiedener Datenreihen und zur Legendenpräsentation
...
:::note{title="Beschreibung"}
Kanäle zur Dimensionskartierung in Wettbewerbsstreudiagrammen
...
:::note{title="Beschreibung"}
Metriken, mindestens 2 Metriken sind erforderlich, um die X- bzw. Y-Achse abzubilden
...
:::note{title="Beschreibung"}
Die Spielerkonfiguration, die zur Angabe der Zeitdimension verwendet wird, ist die Kernkonfiguration des dynamischen Streudiagramms

Die dynamische Aktualisierung der Daten wird durch die Steuerung des Wiedergabefortschritts der Zeitdimension über den Player erreicht
...
:::note{title="Beschreibung"}
Sortierkonfiguration, um zu steuern, wie Dimensionswerte sortiert werden
...
:::note{title="Beschreibung"}
Beschriftungskonfiguration zur Anzeige von Datenbeschriftungen an Datenpunkten
...
:::note{title="Beschreibung"}
XAchsenkonfiguration, bei der es sich um eine numerische Achse handelt, die den ersten metrischen Wert anzeigt
...
:::note{title="Beschreibung"}
YAchsenkonfiguration, bei der es sich um eine numerische Achse handelt, die den zweiten metrischen Wert anzeigt
...
:::note{title="Beschreibung"}
Fadenkreuzkonfiguration, um den genauen Speicherort der Daten anzuzeigen
...
:::note{title="Beschreibung"}
Stilkonfiguration für Datenpunkte; kann als einzelner Stil oder als Array angegeben werden und unterstützt globale oder bedingte Stilkonfigurationen
...
:::note{title="Beschreibung"}
Numerische Callout-Linien, horizontale Callout-Linien, Markierung bestimmter Y-Achsenwerte
