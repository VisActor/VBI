...
Histogramm, geeignet für Längsdatenvergleichsszenarien, X-Achse ist die Kategorieachse (klassifizierte Daten), Y-Achse ist die numerische Achse (kontinuierliche Daten), Spalten sind längs angeordnet
...
:::note{title="Beschreibung"}
Die erste Dimension des Histogramms wird auf die X-Achse abgebildet, Die verbleibenden Dimensionen werden mit dem Metriknamen zusammengeführt (wenn mehrere Metriken vorhanden sind), Als Legendenelement anzeigen
...
:::note{title="Beschreibung"}
Alle Indikatoren des Balkendiagramms werden automatisch zu einem Indikator zusammengeführt, Zuordnen zur Y-Achse, Wenn es mehrere Metriken gibt, Der Metrikname wird mit dem Rest der Dimensionen zusammengeführt, Wird als Legendenelement angezeigt.
