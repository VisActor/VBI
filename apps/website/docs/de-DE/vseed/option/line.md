...
Liniendiagramme unterstützen die folgenden visuellen Kanäle:

`x`: xWellenkanal, Support`mehrere Dimensionen`, Zuordnung zur x-Achse anhand des Dimensionswerts

`y`: yWellenkanal, Support`mehrere Kennzahlen`, Zuordnung zur y-Achse anhand des Kennzahlenwerts
...
Liniendiagramm, geeignet, um den Trend von Daten über die Zeit oder geordnete Kategorien anzuzeigen und eine Trendlinie zu bilden, indem Datenpunkte durch Liniensegmente verbunden werden

Szenario:

\- Trends in Zeitreihendaten anzeigen

\- Trendvergleich über mehrere Datenreihen hinweg vergleichen

\- Analysieren Sie das Wachstum oder den Rückgang von Daten
...
:::note{title="Beschreibung"}
Dimension, Die erste Dimension des Liniendiagramms ist der X-Achse zugeordnet, Die verbleibenden Dimensionen werden mit dem Metriknamen zusammengeführt (wenn mehrere Metriken vorhanden sind), Als Legendenelement anzeigen
...
:::note{title="Beschreibung"}
Kennzahl, Alle Metriken im Liniendiagramm werden automatisch zu einer Metrik zusammengeführt, Zuordnen zur Y-Achse, Wenn es mehrere Metriken gibt, Der Metrikname wird mit dem Rest der Dimensionen zusammengeführt, Wird als Legendenelement angezeigt.
