...
`xAxis`: xWellenkanal, Support`eine Dimension`, Anzeige auf x-Achse nach Berechnung nach Maßwertfeld

:::

:::note{title="Beschreibung"}
Histogramm, geeignet für Szenarien, die die Verteilung von Daten zeigen, wobei die X-Achse die numerische Achse ist (kontinuierliche Daten), die Y-Achse die numerische Achse ist (kontinuierliche Daten) und die Spalten in Längsrichtung angeordnet sind

Szenario:

\- Anzeige der Verteilung von Daten, wie Häufigkeitsverteilung, Wahrscheinlichkeitsverteilung usw.

\- Zentralisierte Trend- und diskrete Analysedaten

\- Identifizieren von Ausreißern und Mustern in Daten
...
:::note{title="Beschreibung"}
Histogramm zur Darstellung der Datenverteilung
...
:::note{title="Beschreibung"}
Histogramme erfordern in der Regel keine Abmessungen
...
:::note{title="Beschreibung"}
Histogramm unterstützt nur eine Dimension und die Daten sind diskrete Daten
...
\- value: Wertekanal für Histogramm
...
:::note{title="Beschreibung"}
Histogramm-Box-Menge, Anzahl der Rechtecke (Spalten), die zur Definition des Histogramms verwendet werden
...
:::note{title="Beschreibung"}
Die Bin-Schrittweite, die zur Berechnung der Bin-Breite verwendet wird, wirkt sich auch auf die Breite des Rechtecks (Spalte) im endgültigen Histogramm aus.Wenn sowohl binCount als auch binStep gesetzt sind, hat binStep Vorrang
...
:::note{title="Beschreibung"}
Histogramm-Behälterwerttyp, Boxed Rechteck (Spalte) Werttyp, der zur Definition des Histogramms verwendet wird, Standardeinstellung:'count'
...
:::note{title="Beschreibung"}
Rechteckiger primitiver Stil, Balkendiagramm-Stilkonfiguration, Definiert den Balkendiagrammstil des Diagramms, Farben für Histogramme einschließen, Rahmen, Abgerundete Ecken usw.
...
:::note{title="Beschreibung"}
Numerische Annotationszeile (Sub-Box-Wert), vertikal angezeigt, kann die Position der Annotationszeile festlegen, Stil usw., wenn Sie möchten, dass die Callout-Zeile dem Feldwert entspricht, können Sie diese Konfiguration verwenden
...
:::note{title="Beschreibung"}
Konfiguration der Kerneldichte-Regressionslinie, Wird verwendet, um Trends und die Verteilung von Daten anzuzeigen
...
:::note{title="Beschreibung"}
Ob die Regressionsgeradenfunktion eingeschaltet ist
...
:::note{title="Beschreibung"}
Empirische kumulative Verteilungsfunktion Regressionslinienkonfiguration, Wird verwendet, um die kumulative Verteilung von Daten anzuzeigen
