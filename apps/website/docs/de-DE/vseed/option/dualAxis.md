...
\- Empfohlene Feldkonfiguration: `2`Metriken, `2`Dimension (en)
...
Das zweiachsige Diagramm unterstützt die folgenden visuellen Kanäle:

`xAxis`: xWellenkanal, Support`mehrere Dimensionen`, Zuordnung zur x-Achse anhand des Dimensionswerts

`primaryYAxis`: Spindel Y-Achsenkanal, Support`mehrere Kennzahlen`, Metriken der Spindel zuordnen

`secondaryYAxis`: Sekundärachse Y-Achsenkanal, Support`mehrere Kennzahlen`, Metriken der sekundären Achse zuordnen
...
Biaxiales Diagramm, geeignet zur Darstellung der Kontrastbeziehung zwischen zwei verschiedenen Größenordnungen oder verschiedenen Einheitenindikatoren, einschließlich der Hauptachse und der Subachse

Szenario:

\- Vergleichende Analyse von Indikatoren unterschiedlicher Größenordnung

\- Trendvergleich von Korrelationsindikatoren

\- Zusammengesetzte Metriken wie Wert und Wachstumsrate müssen gleichzeitig angezeigt werden

\- Unterstützt verschiedene Arten von Diagrammkombinationen (z. B. Liniendiagramm + Balkendiagramm/Liniendiagramm + Flächendiagramm/Flächendiagramm + Balkendiagramm)
...
\- Support-Metrikgruppen, Der erste Indikatorsatz platziert die (Spindel-) linke Achse, Der zweite Satz von Indikatoren platziert die (sekundäre Achse) rechte Achse

\- Die erste Dimension geht auf die X-Achse, Die verbleibenden Dimensionen werden mit dem Metriknamen zusammengeführt (wenn mehrere Metriken vorhanden sind), Wird als Legendenelement angezeigt.

\- Zwei Sätze von Anzeigefeldern können der linken bzw. rechten Y-Achse zugeordnet werden, Alle innerhalb einer Metrikgruppe werden automatisch zu einer Metrik zusammengeführt

Standardmäßig eingeschaltete Funktionen:

\- Achsen, Legenden, Datenbeschriftungen, Eingabeaufforderungen sind standardmäßig aktiviert
...
:::note{title="Beschreibung"}
Biaxiales Diagramm, das ein zusammengesetztes Diagramm der vergleichenden Beziehung zwischen zwei Indikatoren unterschiedlicher Größe zeigt
...
:::note{title="Beschreibung"}
Datensatz, TidyData-konformer und aggregierter Datensatz, der die Datenquelle und die Struktur des Diagramms definiert, Der vom Benutzer eingegebene Datensatz erfordert keine Verarbeitung, VSeedmit leistungsstarken Datenumformungsfunktionen, Wird seine eigenen Daten umgestalten, Die Daten des zweiachsigen Diagramms werden schließlich in 2 Dimensionen konvertiert, 1oder 2 Metriken (je nachdem, ob der Benutzer eine Metrikgruppe konfiguriert hat).

:::

**Beispiel**
```ts
[{month:'1Monat', value:100, growth:0.2}, {month:'2Monat', value:150, growth:0.5}]
...
:::note{title="Beschreibung"}
Dimension, Die erste Dimension geht auf die X-Achse, Die verbleibenden Dimensionen werden mit dem Metriknamen zusammengeführt (wenn mehrere Metriken vorhanden sind), Wird als Legendenelement angezeigt.
...
Für Metriken, die primärenYAxis und sekundärenYAxis in der Codierung zugeordnet sind,

Kann eingestellt werden durch`parentId`Eigenschaften, Gruppieren Sie die Metriken, und die Metriken in verschiedenen Gruppen werden in verschiedenen Untergraphen angezeigt,

Sie können auch`chartType`Attribut, um den Diagrammtyp für verschiedene Metrikgruppen anzugeben.
...
\- primaryYAxis: Haupt-Y-Achse der metrischen Zuordnung

\- secondaryYAxis: Sekundäre y-Achse der metrischen Zuordnung
...
:::note{title="Beschreibung"}
Legen Sie den Diagrammtyp für diese Metrik in einem zweiachsigen Diagramm fest, Gilt nur für Dual-Axis-Diagramme

\- line: Linie

\- column: Balken

\- columnParallel: Paralleles Histogramm

\- columnPercent: Prozent-Balkendiagramm

\- area: Flächendiagramm

\- areaPercent: Prozent Flächendiagramm

\- scatter: Börsen-Diagramm
...
:::note{title="Beschreibung"}
Wird verwendet, um zu definieren, ob die Skalen der beiden Achsen eines zweiachsigen Diagramms ausgerichtet sind, Wenn es mehrere Sätze von Maßnahmen gibt, alignTicksKann als Array konfiguriert werden, Gibt an, ob die Skala eines zweiachsigen Diagramms für jedes Element ausgerichtet ist.
...
:::note{title="Beschreibung"}
Hauptkonfiguration der Y-Achse des zweiachsigen Diagramms, Haupt-Y-Achse zur Definition von zweiachsigen Diagrammen, Position der Y-Hauptachse einschließen, Stil usw., wenn Maßnahmen mehr als eine Gruppe haben, primaryYAxisKann als Array konfiguriert werden, Jedes Element entspricht der Haupt-Y-Achse eines zweiachsigen Diagramms.
...
:::note{title="Beschreibung"}
Sekundäre Y-Achsen-Konfiguration von zweiachsigen Diagrammen, Sekundäre Y-Achse zur Definition von zweiachsigen Diagrammen, Einschließlich Position und Stil der sekundären Y-Achse usw.; wenn Kennzahlen mehr als eine Gruppe haben, kann secondaryYAxis als Array konfiguriert werden, wobei jedes Element einer sekundären Y-Achse des Dual-Axis-Diagramms entspricht.
...
:::note{title="Beschreibung"}
xAchse, Kategorie-Achse, xAchsenkonfiguration, Die zur Definition des Diagramms verwendete x-Achse, Position der x-Achse einschließen, Format, Stil, etc.
...
:::note{title="Beschreibung"}
Diagramm-Hintergrundfarbe, Hintergrundfarbe kann eine Farbstring sein, Beispiel'red', 'blue', Es kann auch Sechskant sein, rgboder rgba'#ff0000', 'rgba(255,0,0,0.5)'
...
Vertikale Tooltip-Konfiguration, Vertikale Eingabeaufforderung zum Definieren eines Diagramms, Enthält die Farbe des vertikalen Eingabefelds, den Etikettenstil usw.
...
:::note{title="Beschreibung"}
Die maximale Breite der Spalte, die entweder ein Pixelwert oder eine prozentuale Zeichenfolge sein kann
...
:::note{title="Beschreibung"}
Abstand zwischen Säulen innerhalb derselben Kategorie; kann ein Pixelwert oder eine Prozentzeichenfolge sein
...
:::note{title="Beschreibung"}
Internationalisierungskonfiguration, Diagrammsprachenkonfiguration, Support'zh\-CN'Und'en\-US'Zweisprachig, Sie können auch intl.setLocale ('zh\-CN') Sprache des Methodensatzes
