...
**vbi-react Starter**`FieldPanel`、`FilterPanel`、`ThemeSelector`、`ChartTypeSelector`、`ChartRenderer` 和 `BuilderLayout` 直接拼出一个可用的低门槛搭建器；需要深度自定义时，再下钻到 hooks。Chart  demo dataUpload CSV**Starter Fields****Dimensions**No dimensions availableAdd- No dimensions selected
...
````
{
  "uuid": "7977d2c7-ff67-43d7-becf-b3b1bdec1e54",
